/**
 * Backfill legal-documents permission modules onto existing system role templates.
 *
 * Usage (from backend/):
 *   npx ts-node -r tsconfig-paths/register scripts/ops/backfill-legal-document-permissions.ts
 */
...
async function main() {
  ...
  console.log(`Backfilled legal-document permissions on ${updated} system role templates.`);
}
