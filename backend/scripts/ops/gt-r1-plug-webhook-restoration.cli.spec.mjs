...
import { parseCliArgs, resolveExecutionMode, PLUG_ID } from './gt-r1-plug-webhook-restoration.lib.mjs';

describe('gt-r1-plug-webhook-restoration CLI gate', () => {
  ...
  it('stays READ_ONLY with only --confirm-webhook', () => {
    const parsed = parseCliArgs([`--confirm-webhook=${PLUG_ID}`]);
    const mode = resolveExecutionMode(parsed);
    assert.equal(mode.mode, 'READ_ONLY');
    assert.equal(mode.authorized, false);
  ...
  it('authorizes mutation only with --execute and exact confirm webhook', () => {
    const parsed = parseCliArgs(['--execute', `--confirm-webhook=${PLUG_ID}`]);
    const mode = resolveExecutionMode(parsed);
