...
const tokenId = parseInt(
  execSync(
    `sudo -u postgres psql -d synqdrive -At -c "SELECT dv.token_id FROM vehicles v INNER JOIN dimo_vehicles dv ON v.dimo_vehicle_id = dv.id WHERE v.dimo_vehicle_id IS NOT NULL AND v.status IN ('AVAILABLE','RENTED') AND dv.connection_status = 'CONNECTED' AND dv.token_id IS NOT NULL ORDER BY dv.token_id LIMIT 1;"`,
    { encoding: 'utf8' },
  ).trim(),
  10,
);
...
const payload = {
  ...
  coolDownPeriod: 30,
  description: 'R9 subscribe probe - delete me',
  displayName: 'SynqDrive R9 Subscribe Probe',
  targetURL: callback,
  status: 'enabled',
  verificationToken,
};
...
const list = (await axios.get(`${api}/v1/webhooks`, { headers })).data?.webhooks ?? [];
const listed = list.find((w) => String(w.id) === String(id));
let subscribe = null;
try {
  await axios.post(`${api}/v1/webhooks/${id}/subscribe/${encodeURIComponent(assetDid)}`, {}, { headers, timeout: 15000 });
  subscribe = { ok: true };
} catch (e) {
  subscribe = { ok: false, status: e.response?.status, data: e.response?.data, message: e.message };
}
try {
  await axios.delete(`${api}/v1/webhooks/${id}/unsubscribe/${encodeURIComponent(assetDid)}`, { headers });
} catch {}
try {
  await axios.delete(`${api}/v1/webhooks/${id}`, { headers });
} catch (e) {
  subscribe.cleanup = e.message;
}
console.log(JSON.stringify({ tokenId, stableId: stableId(id), listed: listed ? { status: listed.status, displayName: listed.displayName, metricName: listed.metricName, failureCount: listed.failureCount } : null, subscribe }, null, 2));
