...
| `twilio-webhook-reachability.sh` | POST probe for `/webhooks/twilio/voice` + `/status` (no real calls) | read-only HTTP check |
...
| `audit-brake-health-baseline-candidates.ts` | Read-only per-component brake baseline backfill candidate audit + controlled apply plan (DRY RUN default) | read-only audit / mutating apply with guards |
...
| `repair-battery-data.ts` | Controlled battery data repair from diagnostic findings (evidence reclassify, REST unverified, publication reset, crank clear, HV dedupe, reference capacity) | mutating — dry-run default; `--apply` required; `--organization-id`, `--vehicle-id`, `--batch-size` |
| `audit-battery-data.ts` | Read-only Battery Health data diagnostic (REST/crank/evidence/publication/HV/reference/write-chain) | read-only — `--organization-id`, `--vehicle-id`, `--format`, `--output`, `--limit`; exit 1 only on runtime errors |
...
| `stations-v2-diagnose.ts` | Read-only Stations V2 data diagnostic (primary invariants, capabilities, coordinates/timezone/hours, vehicle positioning, expected station, archived links, booking rules, scope IDs, KPI drift) | read-only — **`--dry-run` required**; `--organization-id`, `--format`, `--output`, `--limit`, `--include-findings`, `--allow-remote-db`; exit 2 on error-severity findings. **Runbook:** [`docs/runbooks/stations-v2-data-remediation.md`](../../docs/runbooks/stations-v2-data-remediation.md) |
