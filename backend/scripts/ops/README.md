...
| `audit-vehicle-booking-handover-data.ts` | Read-only vehicle / booking / handover consistency diagnostic (raw status vs booking truth, handover lifecycle, reservation window, cross-org links, timezone) | read-only — `--organization-id`, `--vehicle-id`, `--license-plate`, `--format`, `--output`, `--limit`; exit 1 only on runtime errors |
| `repair-vehicle-booking-handover-data.ts` | Controlled repair for unambiguous vehicle/booking/handover status inconsistencies (stale RESERVED/RENTED, handover/booking reconciliation) | mutating — dry-run default; `--apply` required for writes; `--organization-id`, `--vehicle-id`, `--batch-size` |
