/**
 * Backfill granular workflow permission modules onto existing system role templates.
 *
 * Usage (from backend/):
 *   npx ts-node -r tsconfig-paths/register scripts/ops/backfill-workflow-permissions.ts
 */
...
async function main() {
  ...
  console.log(`Backfilled workflow permissions on ${updated} system role templates.`);
}
