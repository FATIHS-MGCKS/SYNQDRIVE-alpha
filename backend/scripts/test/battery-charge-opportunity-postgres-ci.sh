#!/usr/bin/env bash
# M3.3C C2.1 — ephemeral PostgreSQL integration tests for charge-opportunity reader.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

log() { printf '[battery-charge-opportunity-postgres-ci] %s\n' "$*"; }
...
log "charge-opportunity integration tests"
BATTERY_V2_CHARGE_OPPORTUNITY_INTEGRATION=1 npx jest rest-session-charge-context.integration --runInBand

log "battery-charge-opportunity-postgres-ci completed successfully"
