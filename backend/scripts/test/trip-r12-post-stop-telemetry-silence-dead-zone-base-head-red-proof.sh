#!/usr/bin/env bash
# Portable deterministic BASE vs HEAD RED/GREEN proof for post-stop telemetry silence dead-zone.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
BASE_SHA="${BASE_SHA:-df8d9d756d171b24bece564fd705bd570b3d4204}"
DEFAULT_HEAD="$(git -C "$ROOT" rev-parse HEAD)"
HEAD_SHA="${HEAD_SHA:-$DEFAULT_HEAD}"
WORKTREE_ROOT="${TMPDIR:-/tmp}/trip-r12-silence-dead-zone-red-proof-$$"
BASE_DIR="$WORKTREE_ROOT/base"
HEAD_DIR="$WORKTREE_ROOT/head"
PROBE_REL="backend/src/modules/vehicle-intelligence/trips/trip-r12-post-stop-telemetry-silence-dead-zone.postgres-redis.integration.spec.ts"
PROBE_SRC="$ROOT/$PROBE_REL"
METRICS_ROOT="${TMPDIR:-/tmp}/trip-r12-silence-dead-zone-metrics-$$"
...
install_and_run_probe() {
  ...
  local log="/tmp/trip-r12-silence-dead-zone-${label}.log"
  ...
  TRIP_R12_SILENCE_DEAD_ZONE_METRICS_FILE="$metrics_file" \
  TRIP_R12_SILENCE_DEAD_ZONE_PROBE_EXPECT="$expect" \
...
const baseOk =
  ...
  base.BASE_POSSIBLE_END_REACHED === false &&
  base.BASE_ACTIVE_TRIP_REMAINS_OPEN === true;

const headOk =
  !head.missing &&
  head.HEAD_GREEN_PROVEN === true &&
  head.HEAD_POSSIBLE_END_REACHED === true &&
  head.HEAD_END_VALIDATION_REACHED === true &&
  head.HEAD_ACTIVE_KEEP_OPEN_UNBOUNDED === false &&
