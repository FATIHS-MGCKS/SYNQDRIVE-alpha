#!/usr/bin/env bash
# RFRF F4-PR3 ready/promotion gate — isolated localhost PostgreSQL gate (never production).
set -euo pipefail
...
GATE_ID="rfrf_f4_pr3_$(date +%s)"
PG_HOST="${TEST_POSTGRES_HOST:-localhost}"
PG_PORT="${TEST_POSTGRES_PORT:-5432}"
PG_DB="rfrf_f4_pr3_${GATE_ID//-/_}"
PG_USER="rfrf_f4_pr3_${GATE_ID//-/_}_u"
PG_PASS="rfrf_f4_pr3_${GATE_ID}_local"
...
assert_test_db_isolation() {
  ...
  if [[ "${PG_DB}" != rfrf_f4_pr3_* ]]; then
    echo "Refusing: database name must match rfrf_f4_pr3_* (got ${PG_DB})" >&2
    exit 1
  fi
...
(async () => {
  ...
  console.log('RFRF F4-PR3 schema verification OK');
})().catch((error) => {
  console.error('RFRF F4-PR3 schema verification failed:', error.message);
  process.exit(1);
});
NODE
}
...
sync_schema_drift_if_needed() {
  ...
  local log
  log="$(mktemp /tmp/rfrf-f4-pr3-dbpush.XXXXXX.log)"
  set +e
...
export RAW_FUEL_REFUEL_F4_PR3_INTEGRATION=1
...
npm test -- \
  raw-fuel-refuel-fallback-runtime-pr3.postgres.integration.spec.ts \
