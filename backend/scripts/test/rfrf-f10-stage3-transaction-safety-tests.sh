#!/usr/bin/env bash
# RFRF F10.5.0 Stage-3 transaction + recovery safety — fixtures only (no Production mutation).
set -euo pipefail
...
fail() { echo "STAGE3_TEST_FAIL: $*" >&2; exit 1; }
pass() { echo "STAGE3_TEST_PASS: $*"; }
...
assert_stage2_final() {
  local out="$1"
  echo "$out" | grep -q 'STAGE1_FINAL_STATE=STAGE2' || fail "expected STAGE1_FINAL_STATE=STAGE2 after recovery"
  echo "$out" | grep -q 'RECOVERY_TARGET_STAGE=2' || fail "expected RECOVERY_TARGET_STAGE=2"
  rfrf_verify_stage_env_state 2 "$BACKEND_ENV" "$PRODUCTION_CUTOVER" >/dev/null || fail "backend.env not stage 2 after recovery"
}

# Stage-3 expected flags from rollout lib
expected="$(rfrf_stage_expected_flags 3)"
[[ "$expected" == *"master=true"* && "$expected" == *"persist=true"* ]] || fail "unexpected stage3 flags: ${expected}"
echo "STAGE3_EXPECTED_FLAGS=${expected}"
echo "STAGE3_REQUIRED_PREVIOUS_STAGE=2"
...
assert_stage2_final "$out"
pass "restart A failure"
echo "STAGE2_RESTART_A_FAILURE_TEST=PASS"
...
echo "STAGE2_RESTART_B_FAILURE_TEST=PASS"
...
echo "STAGE2_POST_VERIFY_FAILURE_TEST=PASS"
...
assert_stage2_final "$out"
pass "chmod failure"
echo "STAGE2_CHMOD_FAILURE_TEST=PASS"
...
echo "STAGE2_POST_MUTATION_FAIL_TEST=PASS"
...
persist="$(rfrf_parse_permissive_bool "$(rfrf_env_get "$BACKEND_ENV" "$RFRF_FLAG_PERSIST")")"
[[ "$persist" == "false" ]] || fail "PRE incomplete should not enable persist"
pass "PRE evidence incomplete"
echo "STAGE3_PRE_EVIDENCE_FAILURE_TEST=PASS"
...
echo "STAGE2_POST_EVIDENCE_FAILURE_TEST=PASS"
...
assert_stage2_final "$out"
pass "EXP021 drift"
echo "STAGE2_EXP021_DRIFT_TEST=PASS"
...
echo "STAGE2_VDC_AUTHORITY_DRIFT_TEST=PASS"
...
echo "STAGE2_VDC_EPOCH_DRIFT_TEST=PASS"
...
echo "STAGE2_VDC_SHADOW_DECREASE_TEST=PASS"
...
echo "STAGE2_TARGET_ENV_VERIFY_FAILURE_TEST=PASS"
...
echo "STAGE2_RECOVERY_RESTART_A_FAILURE_TEST=PASS"
...
echo "STAGE2_RECOVERY_RESTART_B_FAILURE_TEST=PASS"
...
assert_stage2_final "$out"
echo "STAGE2_RECOVERY_POST_VERIFY_FAILURE_TEST=PASS"

# successful Stage 2 + cutover immutability
reset_stage2_env
clear_inject_flags
...
rc=$?
set -e
(( rc == 0 )) || fail "successful stage3 should exit 0: ${out}"
...
rfrf_verify_stage_env_state 3 "$BACKEND_ENV" "$PRODUCTION_CUTOVER" || fail "stage3 verify failed"
pass "successful stage3"
echo "STAGE3_SUCCESS_FIXTURE_TEST=PASS"

# dry-run 2->3 (byte-identical proof before/after)
reset_stage2_env
clear_inject_flags
export DRY_RUN=1
...
PRE_DRY_RUN_PERSIST="$(rfrf_env_get "$BACKEND_ENV" "$RFRF_FLAG_PERSIST")"
shopt -s nullglob
...
POST_DRY_RUN_PERSIST="$(rfrf_env_get "$BACKEND_ENV" "$RFRF_FLAG_PERSIST")"
...
[[ "$PRE_DRY_RUN_PERSIST" == "$POST_DRY_RUN_PERSIST" ]] || fail "dry run persist flag changed"
shopt -s nullglob
post_backups=( "${BACKEND_ENV}".bak-rfrf-* )
shopt -u nullglob
(( ${#pre_backups[@]} == ${#post_backups[@]} )) || fail "dry run created backup file"
echo "$out" | grep -q 'STAGE3_DRY_RUN_ZERO_MUTATION=PASS' || fail "dry run missing zero mutation"
echo "$out" | grep -q 'STAGE3_DRY_RUN_RESTART_CALLS=0' || fail "dry run missing restart calls marker"
echo "$out" | grep -q 'STAGE3_DRY_RUN_BACKUP_CREATED=NO' || fail "dry run missing backup marker"
echo "$out" | grep -q 'Would set RAW_FUEL_REFUEL_FALLBACK_PERSIST_ENABLED=true' || fail "dry run missing persist=true proposal"
pass "dry run 2->3 byte identical"
echo "STAGE3_DRY_RUN_BYTE_IDENTICAL_PROOF=PASS"
...
# malformed PRE stage (convergence=true at stage 2) must block before mutation
reset_stage2_env
echo "RFRF_NATIVE_FALLBACK_CONVERGENCE_AUTHORIZED=true" >>"$BACKEND_ENV"
clear_inject_flags
...
reset_stage2_env
clear_inject_flags
rfrf_remove_env_key "$BACKEND_ENV" "$RFRF_FLAG_MASTER"
set +e
out="$(run_stage3)"
rc=$?
set -e
...
echo "STAGE3_SKIP_BLOCKED=YES"

# retry stage 3 when already stage 3
reset_stage2_env
clear_inject_flags
run_stage3 >/dev/null
set +e
...
rc=$?
set -e
(( rc != 0 )) || fail "retry at stage 3 should block"
pass "retry stage3 blocked"
echo "STAGE3_RETRY_WHEN_ALREADY_STAGE3_BLOCKED=YES"

echo "STAGE3_RECOVERY_TARGET_STAGE=2"
echo "RECOVERY_RESTORES_PREVIOUS_STAGE=YES"
echo "rfrf-f10-stage3-transaction-safety: OK"
