#!/usr/bin/env bash
# RFRF F10.6.0 Stage-4 rollback fixtures rollback dry-run + fail-closed fixture tests.
set -euo pipefail
...
fail() { echo "STAGE4_ROLLBACK_TEST_FAIL: $*" >&2; exit 1; }
pass() { echo "STAGE4_ROLLBACK_TEST_PASS: $*"; }
...
RAW_FUEL_REFUEL_FALLBACK_PERSIST_ENABLED=true
RFRF_NATIVE_FALLBACK_CONVERGENCE_AUTHORIZED=true
RFRF_FALLBACK_PROMOTION_EXECUTION_AUTHORIZED=true
EOF
}
...
assert_no_production_safe() {
  local out="$1"
  if echo "$out" | grep -q 'STAGE4_ROLLBACK_PRODUCTION_SAFE=YES'; then
    fail "must not emit STAGE4_ROLLBACK_PRODUCTION_SAFE=YES"
  fi
}
...
dry="$(DRY_RUN=1 bash "${OPS}/rfrf-production-rollback.sh" --from-stage 4 2>&1)"
...
# B — Stage 3 source + --from-stage 4 => BLOCKED
write_stage3_env
set +e
wrong="$(DRY_RUN=1 bash "${OPS}/rfrf-production-rollback.sh" --from-stage 4 2>&1)"
rc=$?
set -e
...
write_malformed_stage4_env
set +e
bad="$(DRY_RUN=1 bash "${OPS}/rfrf-production-rollback.sh" --from-stage 4 2>&1)"
rc=$?
set -e
...
run_rollback() {
  bash "${OPS}/rfrf-production-rollback.sh" --from-stage 4 2>&1
}
...
rfrf_verify_stage_env_state 4 "$BACKEND_ENV" "$CUTOVER" >/dev/null || fail "restart A should restore stage 4 file"
pass "rollback restart A failure"
echo "STAGE4_ROLLBACK_RESTART_A_FAILURE_TEST=PASS"
...
if echo "$out" | grep -q 'STAGE4_ROLLBACK_PRODUCTION_SAFE=YES'; then fail "restart B must not emit production safe"; fi
pass "rollback restart B failure"
echo "STAGE4_ROLLBACK_RESTART_B_FAILURE_TEST=PASS"
echo "STAGE4_ROLLBACK_MIXED_AUTHORITY_PREVENTED=YES"
...
if echo "$out" | grep -q 'STAGE4_ROLLBACK_PRODUCTION_SAFE=YES'; then fail "post verify must not emit production safe"; fi
pass "rollback post verify failure"
echo "STAGE4_ROLLBACK_POST_VERIFY_FAILURE_TEST=PASS"
...
echo "STAGE4_ROLLBACK_STAGE_VERIFY_FAILURE_TEST=PASS"
echo "STAGE4_ROLLBACK_STAGE_VERIFY_FAILURE_FAIL_CLOSED=YES"
...
echo "STAGE4_ROLLBACK_RECOVERY_RESTART_A_FAILURE_TEST=PASS"
...
echo "STAGE4_ROLLBACK_RECOVERY_RESTART_B_FAILURE_TEST=PASS"
...
echo "STAGE4_ROLLBACK_RECOVERY_ENV_RESTORE_FAILURE_TEST=PASS"
...
echo "$out" | grep -q 'STAGE4_ROLLBACK_TO_STAGE3_SAFE=YES' || fail "missing stage4 rollback safe marker"
mixed_count="$(echo "$out" | grep -c '^ROLLBACK_MIXED_AUTHORITY_PRESENT=' || true)"
...
rfrf_verify_stage_env_state 3 "$BACKEND_ENV" "$CUTOVER" || fail "post-rollback not stage 3"
pass "stage4 rollback success"
echo "STAGE4_ROLLBACK_TO_STAGE3_SAFE=YES"

echo "rfrf-f10-stage4-rollback-fixture: OK"
