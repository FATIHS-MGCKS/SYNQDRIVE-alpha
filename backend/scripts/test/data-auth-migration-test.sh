#!/usr/bin/env bash
# Data Authorization — PostgreSQL migration tests (empty + legacy altbestand).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

PG_HOST="${DATA_AUTH_MIGRATION_PG_HOST:-127.0.0.1}"
PG_PORT="${DATA_AUTH_MIGRATION_PG_PORT:-5432}"
PG_USER="${DATA_AUTH_MIGRATION_PG_USER:-synqdrive}"
PG_PASSWORD="${DATA_AUTH_MIGRATION_PG_PASSWORD:-synqdrive}"
PG_ADMIN_DB="${DATA_AUTH_MIGRATION_PG_ADMIN_DB:-postgres}"

EMPTY_DB="${DATA_AUTH_MIGRATION_EMPTY_DB:-synqdrive_data_auth_mig_empty}"
LEGACY_DB="${DATA_AUTH_MIGRATION_LEGACY_DB:-synqdrive_data_auth_mig_legacy}"
...
test_legacy_altbestand() {
  ...
  DATABASE_URL="postgresql://${PG_USER}:${PG_PASSWORD}@${PG_HOST}:${PG_PORT}/${LEGACY_DB}?schema=public" \
    ...
    -v ON_ERROR_STOP=1 -f "$ROOT/scripts/test/fixtures/data-authorization-legacy-altbestand.sql"
  run_migrate_deploy "$LEGACY_DB"
  local legacy_count
  legacy_count=$(psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$LEGACY_DB" -tAc \
    "SELECT COUNT(*) FROM org_data_authorizations WHERE status = 'ACTIVE';")
  if [[ "${legacy_count:-0}" -lt 1 ]]; then
    echo "Expected legacy ACTIVE org_data_authorizations >= 1" >&2
    exit 1
  fi
  echo "Legacy altbestand migration OK (${legacy_count} ACTIVE legacy authorizations)"
}
...
case "${1:-all}" in
  ...
  all)
    ...
    test_empty_database
    test_legacy_altbestand
    echo "==> Data authorization migration tests complete"
    ;;
  *)
