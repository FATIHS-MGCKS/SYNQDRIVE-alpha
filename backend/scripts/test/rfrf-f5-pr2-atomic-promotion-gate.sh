#!/usr/bin/env bash
# RFRF F5-PR2 atomic promotion transaction gate — isolated localhost PostgreSQL only.
set -euo pipefail
...
GATE_ID="rfrf_f5_pr2_$(date +%s)"
PG_HOST="${TEST_POSTGRES_HOST:-localhost}"
PG_PORT="${TEST_POSTGRES_PORT:-5432}"
PG_DB="rfrf_f5_pr2_${GATE_ID//-/_}"
PG_USER="rfrf_f5_pr2_${GATE_ID//-/_}_u"
PG_PASS="rfrf_f5_pr2_${GATE_ID}_local"
...
assert_test_db_isolation() {
  ...
  if [[ "${PG_DB}" != rfrf_f5_pr2_* ]]; then
    echo "Refusing: database name must match rfrf_f5_pr2_* (got ${PG_DB})" >&2
    exit 1
  fi
...
(async () => {
  ...
  console.log('RFRF F5-PR2 schema verification OK');
})().catch((error) => {
  console.error('RFRF F5-PR2 schema verification failed:', error.message);
  process.exit(1);
});
NODE
}
...
sync_schema_drift_if_needed() {
  ...
  local log
  log="$(mktemp /tmp/rfrf-f5-pr2-dbpush.XXXXXX.log)"
  set +e
...
npm test -- \
  raw-fuel-refuel-fallback-f5-pr2-promotion.postgres.integration.spec.ts \
