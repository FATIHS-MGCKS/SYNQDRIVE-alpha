#!/usr/bin/env bash
# RFRF F6 — canonical G2 rawDetectionMeta payload compatibility gate (isolated localhost PostgreSQL only).
set -euo pipefail
...
GATE_ID="rfrf_f6_$(date +%s)"
PG_HOST="${TEST_POSTGRES_HOST:-localhost}"
PG_PORT="${TEST_POSTGRES_PORT:-5432}"
PG_DB="rfrf_f6_${GATE_ID//-/_}"
PG_USER="rfrf_f6_${GATE_ID//-/_}_u"
PG_PASS="rfrf_f6_${GATE_ID}_local"
...
assert_test_db_isolation() {
  ...
  if [[ "${PG_DB}" != rfrf_f6_* ]]; then
    echo "Refusing: database name must match rfrf_f6_* (got ${PG_DB})" >&2
    exit 1
  fi
...
(async () => {
  ...
  console.log('RFRF F6 schema verification OK');
})().catch((error) => {
  console.error('RFRF F6 schema verification failed:', error.message);
  process.exit(1);
});
NODE
}
...
sync_schema_drift_if_needed() {
  ...
  local log
  log="$(mktemp /tmp/rfrf-f6-dbpush.XXXXXX.log)"
  set +e
