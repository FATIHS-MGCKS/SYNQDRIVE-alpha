#!/usr/bin/env bash
# RFRF F10.5.0 / F10.4.0.2 Stage-3 rollback dry-run + fail-closed fixture tests.
set -euo pipefail
...
fail() { echo "STAGE3_ROLLBACK_TEST_FAIL: $*" >&2; exit 1; }
pass() { echo "STAGE3_ROLLBACK_TEST_PASS: $*"; }
...
assert_no_production_safe() {
  local out="$1"
  if echo "$out" | grep -q 'STAGE3_ROLLBACK_PRODUCTION_SAFE=YES'; then
    fail "must not emit STAGE3_ROLLBACK_PRODUCTION_SAFE=YES"
  fi
}
...
dry="$(DRY_RUN=1 bash "${OPS}/rfrf-production-rollback.sh" --from-stage 3 2>&1)"
...
# B — Stage 2 source + --from-stage 3 => BLOCKED
write_stage2_env
set +e
wrong="$(DRY_RUN=1 bash "${OPS}/rfrf-production-rollback.sh" --from-stage 3 2>&1)"
rc=$?
set -e
...
write_malformed_stage3_env
set +e
bad="$(DRY_RUN=1 bash "${OPS}/rfrf-production-rollback.sh" --from-stage 3 2>&1)"
rc=$?
set -e
...
run_rollback() {
  bash "${OPS}/rfrf-production-rollback.sh" --from-stage 3 2>&1
}
...
rfrf_verify_stage_env_state 3 "$BACKEND_ENV" "$CUTOVER" >/dev/null || fail "restart A should restore stage 3 file"
pass "rollback restart A failure"
echo "STAGE3_ROLLBACK_RESTART_A_FAILURE_TEST=PASS"
...
if echo "$out" | grep -q 'STAGE3_ROLLBACK_PRODUCTION_SAFE=YES'; then fail "restart B must not emit production safe"; fi
pass "rollback restart B failure"
echo "STAGE3_ROLLBACK_RESTART_B_FAILURE_TEST=PASS"
echo "STAGE3_ROLLBACK_MIXED_AUTHORITY_PREVENTED=YES"
...
if echo "$out" | grep -q 'STAGE3_ROLLBACK_PRODUCTION_SAFE=YES'; then fail "post verify must not emit production safe"; fi
pass "rollback post verify failure"
echo "STAGE3_ROLLBACK_POST_VERIFY_FAILURE_TEST=PASS"
...
echo "STAGE3_ROLLBACK_STAGE_VERIFY_FAILURE_TEST=PASS"
echo "STAGE3_ROLLBACK_STAGE_VERIFY_FAILURE_FAIL_CLOSED=YES"
...
echo "STAGE3_ROLLBACK_RECOVERY_RESTART_A_FAILURE_TEST=PASS"
...
echo "STAGE3_ROLLBACK_RECOVERY_RESTART_B_FAILURE_TEST=PASS"
...
echo "STAGE3_ROLLBACK_RECOVERY_ENV_RESTORE_FAILURE_TEST=PASS"
...
echo "$out" | grep -q 'STAGE3_ROLLBACK_TO_STAGE2_SAFE=YES' || fail "missing stage3 rollback safe marker"
mixed_count="$(echo "$out" | grep -c '^ROLLBACK_MIXED_AUTHORITY_PRESENT=' || true)"
...
rfrf_verify_stage_env_state 2 "$BACKEND_ENV" "$CUTOVER" || fail "post-rollback not stage 2"
pass "stage3 rollback success"
echo "STAGE3_ROLLBACK_TO_STAGE2_SAFE=YES"

echo "rfrf-f10-stage3-rollback-fixture: OK"
