#!/usr/bin/env bash
# RFRF F10.4.0 Stage-2 transaction + recovery safety — fixtures only (no Production mutation).
set -euo pipefail
...
fail() { echo "STAGE2_TEST_FAIL: $*" >&2; exit 1; }
pass() { echo "STAGE2_TEST_PASS: $*"; }
...
assert_stage1_final() {
  local out="$1"
  echo "$out" | grep -q 'STAGE1_FINAL_STATE=STAGE1' || fail "expected STAGE1_FINAL_STATE=STAGE1 after recovery"
  echo "$out" | grep -q 'RECOVERY_TARGET_STAGE=1' || fail "expected RECOVERY_TARGET_STAGE=1"
  rfrf_verify_stage_env_state 1 "$BACKEND_ENV" "$PRODUCTION_CUTOVER" >/dev/null || fail "backend.env not stage 1 after recovery"
}

# Stage-2 expected flags from rollout lib
expected="$(rfrf_stage_expected_flags 2)"
[[ "$expected" == *"master=true"* && "$expected" == *"persist=false"* ]] || fail "unexpected stage2 flags: ${expected}"
echo "STAGE2_EXPECTED_FLAGS=${expected}"
echo "STAGE2_REQUIRED_PREVIOUS_STAGE=1"
...
[[ "$master" == "false" ]] || fail "PRE incomplete should not enable master"
pass "PRE evidence incomplete"
echo "STAGE2_PRE_EVIDENCE_FAILURE_TEST=PASS"
...
rc=$?
set -e
(( rc == 0 )) || fail "successful stage2 should exit 0: ${out}"
...
rfrf_verify_stage_env_state 2 "$BACKEND_ENV" "$PRODUCTION_CUTOVER" || fail "stage2 verify failed"
...
echo "rfrf-f10-stage2-transaction-safety: OK"
