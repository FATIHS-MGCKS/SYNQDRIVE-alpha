...
PG_DB="rfrf_f5_pr1_${GATE_ID//-/_}"
PG_USER="rfrf_f5_pr1_${GATE_ID//-/_}_u"
PG_PASS="rfrf_f5_pr1_${GATE_ID}_local"
...
assert_test_db_isolation() {
  ...
  if [[ "${PG_DB}" != rfrf_f5_pr1_* ]]; then
    echo "Refusing: database name must match rfrf_f5_pr1_* (got ${PG_DB})" >&2
    exit 1
  fi
...
(async () => {
  ...
  console.log('RFRF F5-PR1 schema verification OK');
})().catch((error) => {
  console.error('RFRF F5-PR1 schema verification failed:', error.message);
  process.exit(1);
});
NODE
}
...
sync_schema_drift_if_needed() {
  ...
  local log
  log="$(mktemp /tmp/rfrf-f5-pr1-dbpush.XXXXXX.log)"
  set +e
...
npm test -- \
  raw-fuel-refuel-fallback-f5-pr1-convergence.postgres.integration.spec.ts \
