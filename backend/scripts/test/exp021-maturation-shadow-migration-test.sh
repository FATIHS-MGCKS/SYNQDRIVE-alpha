#!/usr/bin/env bash
# EXP-021 Live Maturation Shadow PR-M1 — PostgreSQL migration deploy proof.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

PG_HOST="${EXP021_SHADOW_MIGRATION_PG_HOST:-127.0.0.1}"
PG_PORT="${EXP021_SHADOW_MIGRATION_PG_PORT:-5432}"
PG_USER="${EXP021_SHADOW_MIGRATION_PG_USER:-exp021_pr1649_test}"
PG_PASSWORD="${EXP021_SHADOW_MIGRATION_PG_PASSWORD:-exp021_pr1649_test_local}"
PG_ADMIN_DB="${EXP021_SHADOW_MIGRATION_PG_ADMIN_DB:-postgres}"

FRESH_DB="${EXP021_SHADOW_MIGRATION_FRESH_DB:-synqdrive_exp021_shadow_mig_fresh}"
CHAIN_DB="${EXP021_SHADOW_MIGRATION_CHAIN_DB:-synqdrive_exp021_shadow_mig_chain}"

export PGPASSWORD="$PG_PASSWORD"

psql_admin() {
  psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$PG_ADMIN_DB" -v ON_ERROR_STOP=1 "$@"
}

wait_for_postgres() {
  local attempts=30
  for ((i = 1; i <= attempts; i++)); do
    if psql_admin -c 'SELECT 1' >/dev/null 2>&1; then
      return 0
    fi
    sleep 2
  done
  echo "PostgreSQL not reachable at ${PG_HOST}:${PG_PORT}" >&2
  return 1
}

recreate_db() {
  local db="$1"
  psql_admin -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${db}' AND pid <> pg_backend_pid();" >/dev/null 2>&1 || true
  psql_admin -c "DROP DATABASE IF EXISTS \"${db}\";"
  psql_admin -c "CREATE DATABASE \"${db}\";"
}

run_canonical_bootstrap() {
  local db="$1"
  export EXP021_FLEET_MIGRATION_PG_HOST="$PG_HOST"
  export EXP021_FLEET_MIGRATION_PG_PORT="$PG_PORT"
  export EXP021_FLEET_MIGRATION_PG_USER="$PG_USER"
  export EXP021_FLEET_MIGRATION_PG_PASSWORD="$PG_PASSWORD"
  export EXP021_FLEET_MIGRATION_DB="$db"
  export DATABASE_URL="postgresql://${PG_USER}:${PG_PASSWORD}@${PG_HOST}:${PG_PORT}/${db}?schema=public"
  bash scripts/test/exp021-canonical-postgres-bootstrap.sh
}

test_fresh_database() {
  echo "==> EXP-021 maturation shadow migration test: fresh database"
  recreate_db "$FRESH_DB"
  run_canonical_bootstrap "$FRESH_DB"
  psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$FRESH_DB" -tAc \
    "SELECT 1 FROM pg_type WHERE typname = 'Exp021MaturationShadowSignalLane';" | grep -q 1
  psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$FRESH_DB" -tAc \
    "SELECT 1 FROM information_schema.tables WHERE table_name = 'exp021_maturation_shadow_window_families';" | grep -q 1
  echo "Fresh DB maturation shadow migration OK"
}

test_existing_chain() {
  echo "==> EXP-021 maturation shadow migration test: deploy after existing chain"
  recreate_db "$CHAIN_DB"
  run_canonical_bootstrap "$CHAIN_DB"
  echo "Existing-chain maturation shadow migration OK"
}

wait_for_postgres

case "${1:-all}" in
  fresh) test_fresh_database ;;
  chain) test_existing_chain ;;
  all)
    test_fresh_database
    test_existing_chain
    ;;
  *)
    echo "Usage: $0 [fresh|chain|all]" >&2
    exit 1
    ;;
esac

echo "EXP-021 maturation shadow migration tests passed."
