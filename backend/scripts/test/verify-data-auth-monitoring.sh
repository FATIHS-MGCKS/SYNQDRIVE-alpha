...
if ! search 'name: synqdrive_data_auth' monitoring/prometheus/alerts.yml; then
...
for metric in \
  ...
  if ! search "$metric" monitoring/prometheus/alerts.yml monitoring/grafana/dashboards/synqdrive-data-authorization.json src/modules/data-authorizations/observability/; then
