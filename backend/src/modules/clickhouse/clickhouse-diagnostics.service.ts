...
import {
  isActivityWindowMirrorEnabled,
  isHfMirrorEnabled,
  isWaypointMirrorEnabled,
...
export interface BuildTableDiagnosticsInput {
  ...
  waypointMirrorEnabled: boolean;
  activityWindowMirrorEnabled: boolean;
...
function buildOneTableDiagnostic(
  ...
  const displayStatus = resolveDisplayStatus(
    ...
    input.waypointMirrorEnabled,
    input.activityWindowMirrorEnabled,
  ...
  if (!input.clickhouseAvailable) {
    ...
    entry.producerStatus === 'active_if_waypoint_mirror_enabled' &&
    !input.waypointMirrorEnabled
  ) {
    notes.push('WAYPOINT_MIRROR_ENABLED=false — waypoint mirror is intentionally off.');
  } else if (
    entry.producerStatus === 'active_if_activity_window_mirror_enabled' &&
    !input.activityWindowMirrorEnabled
  ) {
    notes.push(
      'ACTIVITY_WINDOW_MIRROR_ENABLED=false — activity window producer is intentionally off.',
    );
  } else if (
...
function resolveDisplayStatus(
  ...
  if (entry.producerStatus === 'active_if_hf_enabled') {
    if (!hfMirrorEnabled) return 'active_if_mirror_disabled';
    return dataStatus === 'has_data' ? 'has_data' : 'active_if_hf_enabled';
  }

  if (entry.producerStatus === 'active_if_waypoint_mirror_enabled') {
    if (!waypointMirrorEnabled) return 'active_if_mirror_disabled';
    return dataStatus === 'has_data' ? 'has_data' : 'empty';
  }

  if (entry.producerStatus === 'active_if_activity_window_mirror_enabled') {
    if (!activityWindowMirrorEnabled) return 'active_if_mirror_disabled';
    return dataStatus === 'has_data' ? 'has_data' : 'empty';
  }
...
export class ClickHouseDiagnosticsService {
  ...
  async getDiagnostics(): Promise<ClickHouseDiagnosticsDto> {
    ...
    const waypointMirrorEnabled = isWaypointMirrorEnabled();
    const activityWindowMirrorEnabled = isActivityWindowMirrorEnabled();
    const notes: string[] = [
    ...
    const tables = buildTableDiagnostics({
      ...
      hfMirrorEnabled,
      waypointMirrorEnabled,
      activityWindowMirrorEnabled,
      storageByTable,
    });
