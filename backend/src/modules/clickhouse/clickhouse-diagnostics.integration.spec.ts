...
describe('ClickHouseDiagnosticsService (integration wiring)', () => {
  ...
  it('returns disabled diagnostics when CLICKHOUSE_URL is empty', async () => {
    ...
    expect(dto.hfMirrorEnabled).toBe(false);
  ...
  it('returns available diagnostics with storage stats', async () => {
          ...
          tables: [
            {
              ...
              oldestRecordAt: '2026-07-07T00:00:00.000Z',
              newestRecordAt: '2026-07-08T10:00:00.000Z',
            },
          ],
        },
      }),
    ...
    const dto = await svc.getDiagnostics();
    expect(dto.clickhouseAvailable).toBe(true);
    expect(dto.hfMirrorEnabled).toBe(true);
    expect(dto.schemaMigrations.appliedCount).toBe(4);
    expect(dto.lastMirrorWriteAt.telemetry_snapshots).toBe(
      '2026-07-08T10:00:00.000Z',
    );
    const snap = dto.tables.find((t) => t.table === 'telemetry_snapshots');
    expect(snap?.dataStatus).toBe('has_data');
  });
});
