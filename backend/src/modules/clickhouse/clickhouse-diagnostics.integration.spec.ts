...
describe('ClickHouseDiagnosticsService (integration wiring)', () => {
  ...
  it('includes planned tables without error semantics', async () => {
    ...
    const dto = await svc.getDiagnostics();
    const activityWindows = dto.tables.find((t) => t.table === 'trip_activity_windows');
    expect(activityWindows?.displayStatus).toBe('active_if_mirror_disabled');
    expect(activityWindows?.expectedEmptyAllowed).toBe(true);
    const planned = dto.tables.find((t) => t.table === 'trip_segment_candidates');
