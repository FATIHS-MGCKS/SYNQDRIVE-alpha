...
export const RENTAL_RULES_ACTIVATION_WARNING = {
  ...
  CATEGORY_INACTIVE:
    'Vehicle category is inactive (archived) — category rules are not applied; inheritance falls back to organization defaults and vehicle overrides.',
...
export function buildRentalRulesActivationSnapshot(input: {
  ...
  category: Pick<RentalVehicleCategory, 'id' | 'name' | 'isActive'> | null;
  ...
  const categoryActive = categoryAssigned ? input.category!.isActive : null;
...
export function resolveInactiveCategoryDisplayName(
  category: Pick<RentalVehicleCategory, 'name' | 'isActive'> | null,
): string | null {
  if (!category) return null;
  if (!category.isActive) return `${category.name} (inactive)`;
