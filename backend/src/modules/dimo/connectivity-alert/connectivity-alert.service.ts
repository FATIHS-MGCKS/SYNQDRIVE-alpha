...
export class ConnectivityAlertService {
  ...
  async processResolutionOutboxRow(outboxId: string): Promise<'completed' | 'retry' | 'skipped'> {
    const row = await this.prisma.deviceConnectionEpisodeResolutionOutbox.findUnique({
      where: { id: outboxId },
    });
    if (!row || row.status === DeviceConnectionEpisodeResolutionOutboxStatus.COMPLETED) {
      return 'skipped';
    }

    const claimed = await this.prisma.deviceConnectionEpisodeResolutionOutbox.updateMany({
      where: {
        id: outboxId,
        status: DeviceConnectionEpisodeResolutionOutboxStatus.PENDING,
      },
      data: { status: DeviceConnectionEpisodeResolutionOutboxStatus.PROCESSING },
    });
    ...
    try {
      if (
        ...
        await this.handleAlertResolvePrepared(row);
      }

      await this.prisma.deviceConnectionEpisodeResolutionOutbox.update({
        where: { id: outboxId },
        data: {
      ...
      this.logger.warn(
        `Connectivity alert outbox ${outboxId} failed: ${
          err instanceof Error ? err.message : String(err)
        }`,
      );
      await this.prisma.deviceConnectionEpisodeResolutionOutbox.update({
        where: { id: outboxId },
        data: { status: DeviceConnectionEpisodeResolutionOutboxStatus.PENDING },
      });
      return 'retry';
    }
  }

  async processPendingResolutionOutbox(limit = 25): Promise<number> {
    const rows = await this.prisma.deviceConnectionEpisodeResolutionOutbox.findMany({
      where: { status: DeviceConnectionEpisodeResolutionOutboxStatus.PENDING },
    ...
    for (const row of rows) {
      const result = await this.processResolutionOutboxRow(row.id);
      if (result === 'completed') processed += 1;
    }
    return processed;
  }

  private async handleAlertResolvePrepared(row: {
    ...
    const label = [episode.vehicle.make, episode.vehicle.model]
      .filter(Boolean)
      .join(' ')
      .trim() || row.vehicleId;

    await this.onEpisodeRecovered({
      organizationId: row.organizationId,
      vehicleId: row.vehicleId,
      ...
      recoverySource: mapRecoverySourceToPolicy(recoverySource),
