...
export class CustomerEligibilityService {
  ...
  async evaluateForBooking(
    ...
    this.applyVerificationFromCanonical(verification, policy, result);
    ...
    result.canConfirmBooking =
      ...
      !this.hasNonVerificationConfirmBlockers(customer, policy, result);
    ...
    result.canStartRental =
      ...
      !this.hasNonVerificationPickupBlockers(customer, policy, result);
  ...
  private applyVerificationFromCanonical(
    ...
    for (const reason of verification.blockingReasons) {
    ...
    if (verification.idDocument === 'requires_review') {
      ...
      result.warnings.push('Ausweisprüfung: manuelle Prüfung erforderlich');

      result.requiredActions.push('Ausweisprüfung abschließen');

    }

    if (verification.drivingLicense === 'requires_review') {

      result.warnings.push('Führerscheinprüfung: manuelle Prüfung erforderlich');

      result.requiredActions.push('Führerscheinprüfung abschließen');
    ...
    if (policy.requireVerifiedIdForPickup && verification.idDocument === 'pickup_required') {
      ...
      result.requiredActions.push('Ausweisprüfung beim Pickup durchführen');
    ...
    if (
      ...
      result.requiredActions.push('Führerscheinprüfung beim Pickup durchführen');
  ...
  private hasNonVerificationConfirmBlockers(
  ...
  private hasNonVerificationPickupBlockers(
    ...
    return this.hasNonVerificationConfirmBlockers(customer, policy, result);
