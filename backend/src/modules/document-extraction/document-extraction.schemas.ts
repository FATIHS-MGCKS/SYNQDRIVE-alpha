...
export const DOCUMENT_FIELD_SCHEMAS: Record<ApplyDocumentExtractionType, FieldDef[]> = {
  ...
  FINE: [
    ...
    },
    {
      ...
      label: 'Notice type',
      type: 'enum',
      enumValues: ['PAYMENT_NOTICE', 'HEARING_FORM', 'DRIVER_INQUIRY'],
      hint: 'Distinguishes payment notice vs Anhörungsbogen vs driver inquiry',
    },
    {
      key: 'offenseDateTime',
      label: 'Offense date-time',
      type: 'string',
      hint: 'ISO date-time of the offense (date + time required for attribution)',
    },
    { key: 'eventDate', label: 'Offense date', type: 'date' },
    { key: 'eventTime', label: 'Offense time', type: 'string' },
    { key: 'dueDate', label: 'Payment due date', type: 'date' },
    {
      key: 'responseDeadline',
      label: 'Response deadline',
      type: 'date',
      hint: 'Deadline to respond (Anhörungsbogen)',
    },
    {
      key: 'offenseType',
      label: 'Offense type',
      ...
      hint: 'No default is applied — must be confirmed explicitly',
    },
    {
      key: 'offenseDescription',
      label: 'Offense description',
      type: 'string',
      hint: 'Plain-language violation summary',
    },
    ...
    { key: 'issuingAuthority', label: 'Issuing authority', type: 'string' },
    {
      ...
      type: 'string',
      hint: 'Aktenzeichen / case reference (alias: reportNumber)',
    },
    {
      key: 'feeBreakdown',
      label: 'Fee breakdown',
    ...
    },
    {
      ...
      type: 'string',
      hint: 'Legacy alias for offenseDescription',
    },
    {
      key: 'amountCents',
      label: 'Amount (cents)',
      type: 'number',
      hint: 'Total payable amount in cents (alias: totalCents)',
    },
    {
      key: 'totalCents',
      label: 'Total amount (cents)',
      type: 'number',
      hint: 'Legacy alias for amountCents — multiply EUR by 100 (17,50 EUR → 1750)',
    },
    { key: 'reportNumber', label: 'Reference number (legacy)', type: 'string' },
  ],
  OTHER: [
