 ...
 * Usage: node architecture/vehicle-connectivity/scripts/validate-graph.mjs
 */
import fs from 'fs';
...
const graphDir = path.join(repo, 'architecture/vehicle-connectivity/graph');
const authorityDir = path.join(repo, 'architecture/vehicle-connectivity');
...
function parseKnowledgeGraphDecisionTable(filePath) {
  ...
  if (!fs.existsSync(filePath)) return map;
  const text = fs.readFileSync(filePath, 'utf8');
  const rowRe = /\| (VC-DEC-[A-Z0-9-]+) \|[^|]+\| ([A-Z_]+(?: \([^)]+\))?) \|/g;
...
function parseDecisionRegisterEvidenceFields(filePath) {
  ...
  for (const m of text.matchAll(sectionRe)) {
    ...
    entries.push({
      ...
      refs: primaryMatch
        ? [...primaryMatch[1].matchAll(/VC-[A-Z0-9-]+/g)].map((x) => x[0])
        : [],
    });
  }
  return entries;
...
function parseDecisionRegisterSectionIds(filePath) {
  ...
  const text = fs.readFileSync(filePath, 'utf8');
  for (const m of text.matchAll(/## (VC-DEC-[A-Z0-9-]+)/g)) ids.push(m[1]);
  return ids;
}
...
function assertEvidenceReferenceAllowed(ownerLabel, ref, nodeById, evidenceRefTypes) {
  ...
  if (!ref.startsWith('VC-EVID-') && !ref.startsWith('VC-TEST-')) {
    fail(`${ownerLabel} evidence ref ${ref} must use VC-EVID-* or VC-TEST-* prefix`);
  }
}
...
for (const name of ['nodes.yaml', 'edges.yaml', 'invariants.yaml']) {
  load(name);
  console.log('  OK architecture/vehicle-connectivity/graph/' + name);
}
...
for (const inv of invariants) {
  ...
  if (!inv.id.startsWith('VC-INV-')) fail(`Invariant ${inv.id} must use VC-INV- prefix`);
...
const gapPattern = /VC-GAP-[A-Z0-9-]+/g;
const hypPattern = /VC-HYP-[A-Z0-9-]+/g;
const contraPattern = /VC-CX-[0-9]+/g;
const supersededPattern = /VC-SUPERSEDED-[A-Z0-9-]+/g;
const rejectPattern = /VC-REJECT-[A-Z0-9-]+/g;
...
for (const decisionId of registerDecisionIds) {
  ...
  if (entry.refs.length === 0) {
    fail(`DECISION_REGISTER ${decisionId} EVIDENCE field has no VC-* references`);
    continue;
  }
