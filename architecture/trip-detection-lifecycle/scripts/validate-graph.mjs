...
const graphDir = path.join(repo, 'architecture/trip-detection-lifecycle/graph');
const authorityDir = path.join(repo, 'architecture/trip-detection-lifecycle');
...
function parseDecisionStatusesFromMarkdown(filePath) {
  ...
  const sectionRe = /## (TDL-DEC-[A-Z0-9-]+)[^\n]*\n[\s\S]*?\| \*\*STATUS\*\* \| ([^|]+) \|/g;
...
function parseKnowledgeGraphDecisionTable(filePath) {
  ...
  if (!fs.existsSync(filePath)) return map;
  const text = fs.readFileSync(filePath, 'utf8');
  const rowRe = /\| (TDL-DEC-[A-Z0-9-]+) \|[^|]+\| ([A-Z_]+(?: \([^)]+\))?) \|/g;
...
function parseDecisionRegisterEvidenceFields(filePath) {
  ...
  for (const m of text.matchAll(sectionRe)) {
    ...
    entries.push({
      ...
      refs: primaryMatch
        ? [...primaryMatch[1].matchAll(/TDL-[A-Z0-9-]+/g)].map((x) => x[0])
        : [],
    });
  }
  return entries;
...
function parseDecisionRegisterSectionIds(filePath) {
  ...
  const text = fs.readFileSync(filePath, 'utf8');
  for (const m of text.matchAll(/## (TDL-DEC-[A-Z0-9-]+)/g)) ids.push(m[1]);
  return ids;
}
...
function assertEvidenceReferenceAllowed(ownerLabel, ref, nodeById, evidenceRefTypes) {
  ...
  if (!ref.startsWith('TDL-EVID-') && !ref.startsWith('TDL-TEST-')) {
    fail(`${ownerLabel} evidence ref ${ref} must use TDL-EVID-* or TDL-TEST-* prefix`);
  }
}
...
for (const name of ['nodes.yaml', 'edges.yaml', 'invariants.yaml']) {
  load(name);
  console.log('  OK architecture/trip-detection-lifecycle/graph/' + name);
}
...
for (const inv of invariants) {
  ...
  if (!inv.id.startsWith('TDL-INV-')) fail(`Invariant ${inv.id} must use TDL-INV- prefix`);
...
const gapPattern = /TDL-GAP-[A-Z0-9-]+/g;
const hypPattern = /TDL-HYP-[A-Z0-9-]+/g;
...
for (const decisionId of registerDecisionIds) {
  ...
  if (entry.refs.length === 0) {
    fail(`DECISION_REGISTER ${decisionId} EVIDENCE field has no TDL-* references`);
    continue;
  }
