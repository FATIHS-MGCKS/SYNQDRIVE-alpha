...
| TDL-TEST-R12-PEC-EV-003 | CURRENT_TEST | [trip-r12-pec-ev-base-head-red-probe.postgres-redis.integration.spec.ts](../../../backend/src/modules/vehicle-intelligence/trips/trip-r12-pec-ev-base-head-red-probe.postgres-redis.integration.spec.ts) | `2026-09-11T21:40:00Z` | portable probe + `test:trip-r11:postgres-redis:ci` @ `9f8078cd6…` | BASE-compatible probe; JSON metrics; worktree proof; validation env fix | **CI_PENDING** (await Trip FSM run on `9f8078cd6…`) | via `test:trip-r12:pec-ev:base-head-red-proof`; causal BASE RED observed run 34647969555 (validation env bug fixed `9f8078cd6`) |
