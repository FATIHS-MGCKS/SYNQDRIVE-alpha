...
| **VDC-CX-010** | Equal sourceTimestamp accepted as full VLS upsert | CONTRADICTED | `isIncomingVlsSourceTimestampStale` rejects only `incoming < existing`. Equality performs full VLS upsert and may execute downstream side effects without source-time advance. Documented only — **not fixed** in Phase 1. See [signals/SIGNAL_AUTHORITY.md](../signals/SIGNAL_AUTHORITY.md), VDC-Q-009. |
