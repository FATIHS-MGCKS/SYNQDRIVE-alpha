...
| **CHANGE** | Adopt subphases P2.1–P2.7: new physical-state action outbox; shadow compare; evidence writers; pre-seed; authority gate; side-effect execution; pilot rollout. Snapshot physical reconcile **before** VLS monotonic early return. Event history becomes evidence-only after authority cutover. **AUTHORITY_CUTOVER_IS_FORWARD_ONLY.** Minimum four flag dimensions (write, shadow, authority, side-effects), all default OFF. |
