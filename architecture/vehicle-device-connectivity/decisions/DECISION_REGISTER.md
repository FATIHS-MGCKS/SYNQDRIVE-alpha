...
| **CHANGE** | Adopt subphases P2.1–P2.7: coordinator-owned atomic tx (Option A); new physical-state action outbox; **STATEFUL_SHADOW** required; evidence writers; pre-seed (time-wins, not source-priority); authority gate; side-effect execution with legacy snapshot resolver off; pilot rollout. Snapshot APPLIED PLUG emits `resolve_plug` even under selfHeal. Snapshot UNPLUG never opens episode. Event history **APPLIED-only**. Drift adjudication: UNEXPLAINED divergences = 0, not all OLD_REJECT_NEW_ACCEPT = 0. **AUTHORITY_CUTOVER_IS_FORWARD_ONLY.** Master flag + four sub-dimensions, all default OFF. |
