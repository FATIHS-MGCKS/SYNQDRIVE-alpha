...
| **Last updated** | 2026-09-22 (isolated multi-profile OBD PLUG canary ops — WOB + KS MX 2024; P2.5 pilot T0 unchanged) |
...
- **OBD PLUG isolated canary (topology A, 2026-09-21/22):** Allowlisted per-vehicle temp PLUG webhooks via `gt-r1-plug-webhook-canary-isolated.mjs` — profiles **WOB_L_7503** (`192922`) and **KS_MX_2024** (`187336`, Mercedes C63 LTE_R1); each profile has unique display name, confirmation token, and profile-scoped activate/teardown; **legacy global PLUG remains disabled**; SNAPSHOT_OBD fallback unchanged. WOB canary activated Production 2026-09-21; KS MX activation gated on deploy + operator confirm `vdc-ks-mx-2024-plug-isolated-canary`.
