...
| **Last updated** | 2026-09-25 (P2.5 T+7 semantic closure Option C on PR #1697 — domain-aware shadow comparator + non-isomorphic same-state admissibility; T7 epoch remains pre-fix; no Production deploy) |
...
- **RB-019 Phase 2 P2.5 T+7 shadow semantic closure Option C (IMPLEMENTED — PR #1697, NOT Production-deployed):** T7 audit **1552/1552** raw `UNEXPLAINED_OLD_REJECT_NEW_ACCEPT` rows are **same-state provenance refresh** (P1A=985, P1B=148, P2=416, P3=3); **zero** true state disagreement. Domain-aware comparator precedence (CONFLICT → binding → strict `STATE_TRANSITION` → proven same-state → decision-pair). Future cutover gates use **correctness-blocking** metrics (`CORRECTNESS_BLOCKING_UNEXPLAINED`, `UNPROVEN_SAME_STATE_REFRESH`, etc.) — not raw unexplained count alone. **T7 seven-day epoch remains pre-fix**; new full post-fix epoch required after deploy. **Build attestation mismatch remains cutover blocker** until post-fix SHA re-attested (`POST_FIX_BUILD_ATTESTATION_REISSUE_REQUIRED=YES`). Evidence: [evidence/P25_T7_SHADOW_COMPARATOR_SEMANTIC_CLOSURE_2026-09-25.md](evidence/P25_T7_SHADOW_COMPARATOR_SEMANTIC_CLOSURE_2026-09-25.md) (VDC-EVID-P25-T7-SEMANTIC-CLOSURE-001); decision **VDC-DEC-014**.
