...
| **Last updated** | 2026-09-23 (WOB L 7503 long-stand wake-associated PLUG GT documented; KS MX parked GT proven; dual isolated PLUG canaries live; P2.5 T0 unchanged) |
...
- **RB-019 Phase 2 P2.5 post-bootstrap provenance refresh GT-R1 (IMPLEMENTED — branch candidate, NOT Production-deployed):** `SNAPSHOT_PLUG_POST_BOOTSTRAP_PROVENANCE_REFRESH` for established PLUGGED projection + newer same-state SNAPSHOT_OBD + legacy `no_open_episode` + coordinator `PROVENANCE_REFRESH`; reuses `EXPECTED_FIX_OLD_REJECT_NEW_ACCEPT` when proven; shadow domain helper `inferPhysicalStateShadowComparisonDomain`; **does not rewrite** T+24h historical `UNEXPLAINED_*` rows. Evidence: [evidence/P25_POST_BOOTSTRAP_PROVENANCE_REFRESH_GT_R1_2026-09-19.md](evidence/P25_POST_BOOTSTRAP_PROVENANCE_REFRESH_GT_R1_2026-09-19.md).
