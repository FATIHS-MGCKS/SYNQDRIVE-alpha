...
| **Last updated** | 2026-09-25 (Option C Production deploy `8a1d9c6586cbddc41bb6c94870f9d51226d71aa2`; post-fix shared epoch + passive LTE_R1 cadence observer started — see [evidence/P25_POST_FIX_LTE_R1_EPOCH_START_2026-09-25.md](evidence/P25_POST_FIX_LTE_R1_EPOCH_START_2026-09-25.md)) |
| **Production runtime SHA (connectivity)** | `8a1d9c6586cbddc41bb6c94870f9d51226d71aa2` (release `20260925182907_v4994`) |
...
- **KS MX 2024 parked UNPLUG→PLUG GT (2026-09-22):** Controlled physical cycle with operator GT — UNPLUG via legacy webhook (`18:02:03.897Z` physical apply); replug with fresh provider OBD at `18:27:15Z` while **RESTING** (clean parked sleep). **SynqDrive:** webhook reached physical reconciliation first (`18:27:17.829Z` **APPLIED**); snapshot `PROVENANCE_REFRESH` `18:27:19.662Z`; HTTP→physical **61 ms**; webhook lead over snapshot reconciliation **1833 ms**. **Provider visibility** operator→first OBD plug evidence **191 s** vs prior KS MX parked GT **~57148 s** — improvement **not** attributable to webhook alone (`CAUSAL_ATTRIBUTION_295X_TO_WEBHOOK_ALONE=NO`). Legacy `resolvedAt` `18:27:15Z` is provider observation time, not DB processing; episode closed via **SNAPSHOT_PLUG_SIGNAL** while physical winner was **WEBHOOK**. Evidence: [evidence/PLUG_WEBHOOK_KS_MX_PARKED_GT_2026-09-22.md](evidence/PLUG_WEBHOOK_KS_MX_PARKED_GT_2026-09-22.md) (VDC-EVID-PLUG-WEBHOOK-KS-MX-PARKED-GT-001). **KS_MX_PARKED_GT:** `PARKED_RECOVERY_PROVEN`.
- **WOB L 7503 long-stand UNPLUG→parked REPLUG→wake-associated PLUG GT (2026-09-23):** Operator UNPLUG `07:41:20Z` / replug `08:05:38Z` after **47752 s** `LONG_STAND_CLEAN` stand. **Parked phase** (through trip `start_time` `10:02:00Z`): **no** provider PLUG, **no** PLUG webhook, **no** fresh snapshot OBD, **no** physical PLUG (**6982000 ms** observation; **21** successful polls, **0** fresh OBD) — `PARKED_REPLUG_RECOVERY_OCCURRED=NO`. **Post activation:** first provider PLUG `10:02:56Z` (**56 s** after trip start; **not** ignition-causal claim); webhook **APPLIED** `10:03:03.936Z` (**146 ms** HTTP→apply); snapshot `PROVENANCE_REFRESH` `10:06:12.499Z` (**188563 ms** after webhook reconciliation). Pre-unplug spurious temp PLUG `07:41:49Z` → `PROVENANCE_REFRESH` only. Cross-vehicle parked behavior **heterogeneous** vs KS MX **191 s** parked visibility. Evidence: [evidence/PLUG_WEBHOOK_WOB_LONG_STAND_WAKE_GT_2026-09-23.md](evidence/PLUG_WEBHOOK_WOB_LONG_STAND_WAKE_GT_2026-09-23.md) (VDC-EVID-PLUG-WEBHOOK-WOB-LONG-STAND-WAKE-GT-001). **WOB_LONG_STAND_GT:** `PARKED_RECOVERY_NOT_OBSERVED_WAKE_ASSOCIATED_RECOVERY_PROVEN`. **Global PLUG rollout:** `DEFERRED_TO_T7_REVIEW` (do not enable legacy global PLUG yet).
...
- **RB-019 Phase 2 P2.5 post-bootstrap provenance refresh GT-R1 (IMPLEMENTED — branch candidate, NOT Production-deployed):** `SNAPSHOT_PLUG_POST_BOOTSTRAP_PROVENANCE_REFRESH` remains a channel-specific GT-R1 proof subtype; steady-state same-state refresh is classified via **Option C** (`NON_ISOMORPHIC_SAME_STATE_PROVENANCE_REFRESH` + mandatory online admissibility) when `proveNonIsomorphicSameStateProvenanceRefresh` passes — distinct from `EXPECTED_FIX_*` (legacy wrong vs dual valid domains). Evidence: [evidence/P25_POST_BOOTSTRAP_PROVENANCE_REFRESH_GT_R1_2026-09-19.md](evidence/P25_POST_BOOTSTRAP_PROVENANCE_REFRESH_GT_R1_2026-09-19.md).
- **RB-019 Phase 2 P2.5 T+7 shadow semantic closure Option C (Production-deployed @ `8a1d9c65`):** T7 audit **1552/1552** pre-fix rows classified as same-state provenance refresh; post-fix **seven-day** observation uses shared canonical **NEW_T0** (manifest on VPS shared evidence path). **Pre-fix epoch `2026-09-18T09:33:25Z` → `2026-09-25T09:33:25Z` retained as historical evidence.** Cutover-capable build attestation reissued to deployed SHA; authority/side effects remain **OFF**. Evidence: [evidence/P25_T7_SHADOW_COMPARATOR_SEMANTIC_CLOSURE_2026-09-25.md](evidence/P25_T7_SHADOW_COMPARATOR_SEMANTIC_CLOSURE_2026-09-25.md); epoch start: [evidence/P25_POST_FIX_LTE_R1_EPOCH_START_2026-09-25.md](evidence/P25_POST_FIX_LTE_R1_EPOCH_START_2026-09-25.md) (VDC-EVID-P25-T7-SEMANTIC-CLOSURE-001); decision **VDC-DEC-014**.
