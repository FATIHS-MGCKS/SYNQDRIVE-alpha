 ...
 * Usage: node architecture/vehicle-device-connectivity/scripts/validate-graph.mjs
 */
import fs from 'fs';
...
const graphDir = path.join(repo, 'architecture/vehicle-device-connectivity/graph');
const authorityDir = path.join(repo, 'architecture/vehicle-device-connectivity');
...
function parseDecisionStatusesFromMarkdown(filePath) {
  ...
  const sectionRe = /## (VDC-DEC-[A-Z0-9-]+)[^\n]*\n[\s\S]*?\| \*\*STATUS\*\* \| ([^|]+) \|/g;
...
function parseKnowledgeGraphDecisionTable(filePath) {
  ...
  if (!fs.existsSync(filePath)) return map;
  const text = fs.readFileSync(filePath, 'utf8');
  const rowRe = /\| (VDC-DEC-[A-Z0-9-]+) \|[^|]+\| ([A-Z_]+(?: \([^)]+\))?) \|/g;
...
function parseDecisionRegisterEvidenceFields(filePath) {
  ...
  for (const m of text.matchAll(sectionRe)) {
    ...
    entries.push({
      ...
      refs: primaryMatch
        ? [...primaryMatch[1].matchAll(/VDC-[A-Z0-9-]+/g)].map((x) => x[0])
        : [],
    });
  }
  return entries;
...
function parseDecisionRegisterSectionIds(filePath) {
  ...
  const text = fs.readFileSync(filePath, 'utf8');
  for (const m of text.matchAll(/## (VDC-DEC-[A-Z0-9-]+)/g)) ids.push(m[1]);
  return ids;
}
...
function assertEvidenceReferenceAllowed(ownerLabel, ref, nodeById, evidenceRefTypes) {
  ...
  if (!ref.startsWith('VDC-EVID-') && !ref.startsWith('VDC-TEST-')) {
    fail(`${ownerLabel} evidence ref ${ref} must use VDC-EVID-* or VDC-TEST-* prefix`);
  }
}
...
for (const name of ['nodes.yaml', 'edges.yaml', 'invariants.yaml']) {
  load(name);
  console.log('  OK architecture/vehicle-device-connectivity/graph/' + name);
}
...
for (const inv of invariants) {
  ...
  if (!inv.id.startsWith('VDC-INV-')) fail(`Invariant ${inv.id} must use VDC-INV- prefix`);
...
const gapPattern = /VDC-GAP-[A-Z0-9-]+/g;
const hypPattern = /VDC-HYP-[A-Z0-9-]+/g;
const contraPattern = /VDC-CX-[0-9]+/g;
const supersededPattern = /VDC-SUPERSEDED-[A-Z0-9-]+/g;
const rejectPattern = /VDC-REJECT-[A-Z0-9-]+/g;
...
for (const decisionId of registerDecisionIds) {
  ...
  if (entry.refs.length === 0) {
    fail(`DECISION_REGISTER ${decisionId} EVIDENCE field has no VDC-* references`);
    continue;
  }
