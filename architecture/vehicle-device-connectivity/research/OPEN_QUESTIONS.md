...
| **VDC-Q-013** | Aug 2026 enqueue_failed + ~100 min delay? | 2 | PARTIAL | **PARTIALLY_ANSWERED** | Mechanism **CONFIRMED**: BullMQ `jobId` colon bug → HTTP 5xx to DIMO; scheduler retry after fix `655f9dbe`; ~100 min delay | VDC-DEC-006; VDC-EVID-GT-R1-UNPLUG-FAILURE-001 | VDC/ops |
| **VDC-Q-015** | Does DIMO UNPLUG webhook `failed` auto-recover on successful delivery, or require explicit `PUT` enable? Does `PUT` reset `failureCount`? | GT recovery | PROVIDER_OBSERVATION | **PARTIALLY_ANSWERED** | Auto-recovery without `PUT` **UNKNOWN**; `PUT` enable **VERIFIED**; **live delivery post-recovery CONFIRMED** (GT-R1 inbox row `70cb73e7`, `failureCount=0`) | VDC-EVID-GT-R1-UNPLUG-RECOVERY-001; VDC-EVID-GT-R1-EXECUTION-001 | DIMO Integration |
| **VDC-Q-016** | How should canonical OBD plug-state be repaired when PLUG webhook disabled and last event is UNPLUGGED after SNAPSHOT_PLUG_SIGNAL resolution? | GT-R1 | PRODUCTION_OBSERVATION + CODE | **PARTIALLY_ANSWERED** | Phase 1 foundation validated (projection + self-heal policy); **live runtime defect unrepaired** (flag OFF, gate unwired); Phase 2 scoped in VDC-DEC-013 with snapshot PLUG `resolve_plug` contract — closes only after runtime cutover + GT-R1 cutover validation | VDC-RB-019 P2; VDC-DEC-012/013 | VDC |
| **VDC-Q-017** | Minimum safe flag model for staged physical-state cutover (write vs authority vs side-effects)? | Phase 2 | ARCHITECTURE | **PROPOSED** | Master kill switch retained; four sub-dimensions; effective equations in hardened audit §13 — VDC-DEC-013 | VDC-RB-019 P2 | VDC |
| **VDC-Q-018** | How to remediate existing open episodes when pre-seed baseline disagrees? | Phase 2 | UNKNOWN | **OPEN** | Pre-seed must not silently close/open; separate recovery step required | VDC-RB-019 P2.4+ | VDC |
