...
| **VDC-Q-015** | Does DIMO UNPLUG webhook `failed` auto-recover on successful delivery, or require explicit `PUT` enable? Does `PUT` reset `failureCount`? | GT forensics | PROVIDER_OBSERVATION | **OPEN** | `failed` + `failureCount=11` stale since 2026-08-25 despite callback recovery; auto-recovery **UNKNOWN** | GT-R1 gate; authorized PUT proposed | DIMO Integration + operator |
