...
- Live 10/20/30/60s HF calibration — **EXP-019 + EXP-020 complete**; **EXP-021 UPPER_BOUND_V2** first physical run **2026-09-11** (KS MS 661, evidence frozen `evidence/reference-capture/EXP_021_KS_MS_661_UPPER_BOUND_V2_EVIDENCE_FREEZE_2026-09-11.md`); 180/120 **DEGRADED_INSUFFICIENT_REQUESTS** (~4/3 HF polls vs 5 slots); 60/30 invalid (operator stopped ~26.2 min); settlement FIXED_INTERVAL 48/48; gap join 4/44 assessable under legacy A/B geometry; post-run hardening adds deterministic request slots, T0 poll reset, physical-end-early terminalization, tiled settlement coverage (~150 queries/run), `validMovementDurationMs` persistence; WHOLE_TRIP **BLOCKED_BY_EXTERNAL_TRIP_FSM_WORKSTREAM**
