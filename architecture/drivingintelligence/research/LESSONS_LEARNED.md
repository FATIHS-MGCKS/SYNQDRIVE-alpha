...
| LL-024 | **Larger HF query windows do not improve settled union when tiling the same span** — EXP-020 W060–W300 identical (141 buckets). Production post-trip already uses whole-trip maximal window. Live gap problem is settlement delay (~27s P50 first-obs) + provider sparsity, not window size alone. | EXP-020 | — |
| LL-025 | **Do not use cross-geometry bucket unions as completeness denominators** — EXP-020 “446” union mixes T+30 incremental obs + whole-trip + whole-session with ms-key disjointness (~36 floor-second overlaps hidden). Per-probe +600s reference required. | EXP-021 preflight | — |
