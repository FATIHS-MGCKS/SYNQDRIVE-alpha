...
| **FIRST_LIVE_DRIVE_FORENSIC** | Trip `a72fb179-3fca-42a1-bdc1-3461cbcade44` (token `187336`) remains **without** `physicalDriveInterval` — preserved as defect evidence; **no** backfill, **no** retroactive PDI on deploy. |
| **OFFICIAL_M2_FOR_FIRST_DRIVE** | **Uncollected** — no maturation WindowFamily/attempts for that drive; separate read-only **FORENSIC_60_90** reconstruction does not create official M2 results. |
| **POST_FIX_VALIDATION** | First valid post-repair maturation candidate = **next** KS MX 2024 physical drive after merge + deploy + new `EXP021_CANARY_LIVE_WINDOW_ACTIVATION_NOT_BEFORE_ISO`. |
| **PDI_AUTHORITY_WHY** | `CANARY_VEHICLE_TRIP_CONFIRMED` — trip-bound canary uses FSM-persisted `vehicle_trips.start_time` / `end_time` for ledger `vehicle_trip_id`; not `ORCHESTRATOR_CONFIRMED` (autonomous CLI path) nor `PDI_CANDIDATE` (motion overlay). Idempotent merge via `persistPhysicalDriveIntervalAuthority`; conflicting interval → fail-closed log, no overwrite. |
