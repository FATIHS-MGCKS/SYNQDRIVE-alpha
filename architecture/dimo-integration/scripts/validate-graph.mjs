 ...
 * Usage: node architecture/dimo-integration/scripts/validate-graph.mjs
 */
import fs from 'fs';
...
const graphDir = path.join(repo, 'architecture/dimo-integration/graph');
const authorityDir = path.join(repo, 'architecture/dimo-integration');
...
function parseDecisionStatusesFromMarkdown(filePath) {
  ...
  const sectionRe = /## (DIM-DEC-[A-Z0-9-]+)[^\n]*\n[\s\S]*?\| \*\*STATUS\*\* \| ([^|]+) \|/g;
...
function parseKnowledgeGraphDecisionTable(filePath) {
  ...
  if (!fs.existsSync(filePath)) return map;
  const text = fs.readFileSync(filePath, 'utf8');
  const rowRe = /\| (DIM-DEC-[A-Z0-9-]+) \|[^|]+\| ([A-Z_]+(?: \([^)]+\))?) \|/g;
...
function parseDecisionRegisterEvidenceFields(filePath) {
  ...
  for (const m of text.matchAll(sectionRe)) {
    ...
    entries.push({
      ...
      refs: primaryMatch
        ? [...primaryMatch[1].matchAll(/DIM-[A-Z0-9-]+/g)].map((x) => x[0])
        : [],
    });
  }
  return entries;
...
function parseDecisionRegisterSectionIds(filePath) {
  ...
  const text = fs.readFileSync(filePath, 'utf8');
  for (const m of text.matchAll(/## (DIM-DEC-[A-Z0-9-]+)/g)) ids.push(m[1]);
  return ids;
}
...
function assertEvidenceReferenceAllowed(ownerLabel, ref, nodeById, evidenceRefTypes) {
  ...
  if (!ref.startsWith('DIM-EVID-') && !ref.startsWith('DIM-TEST-')) {
    fail(`${ownerLabel} evidence ref ${ref} must use DIM-EVID-* or DIM-TEST-* prefix`);
  }
}
...
for (const name of ['nodes.yaml', 'edges.yaml', 'invariants.yaml']) {
  load(name);
  console.log('  OK architecture/dimo-integration/graph/' + name);
}
...
for (const inv of invariants) {
  ...
  if (!inv.id.startsWith('DIM-INV-')) fail(`Invariant ${inv.id} must use DIM-INV- prefix`);
...
const gapPattern = /DIM-GAP-[A-Z0-9-]+/g;
const hypPattern = /DIM-HYP-[A-Z0-9-]+/g;
const contraPattern = /DIM-CX-[0-9]+/g;
const supersededPattern = /DIM-SUPERSEDED-[A-Z0-9-]+/g;
const rejectPattern = /DIM-REJECT-[A-Z0-9-]+/g;
...
for (const decisionId of registerDecisionIds) {
  ...
  if (entry.refs.length === 0) {
    fail(`DECISION_REGISTER ${decisionId} EVIDENCE field has no DIM-* references`);
    continue;
  }
