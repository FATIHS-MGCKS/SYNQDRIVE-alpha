...
|----|-------|---------|-----------|--------------|----------------|---------------------|---------------|----------|-----------|-------------------|------------|-----------------|----------|------------|-------|----------|--------------|
| `BAT-V2-GAP-LV-CANONICAL-ASSESSMENT-HANDOFF-001` | REST complete → no assessment enqueue | A | — | CONFIRMED | Stage-2 cutover blocker when flags ON | NONE | CONFIRMED missing enqueue in REST handler | **P0_ACTIVATION_BLOCKER** | IMPLEMENTATION_READY | Hybrid direct enqueue + reconciliation safety net | D1/D2/D3 VALIDATED; PKG-01 ready for impl | Integration + flag-gated soak | YES | Maybe (reconcile index) | YES | LOW | Runtime PKG-01 |
| `BAT-V2-GAP-LV-PUBLICATION-HANDOFF-001` | Assessment complete → no publication enqueue | A | — | CONFIRMED | Publication path incomplete when flags ON | NONE | CONFIRMED handler path when manually invoked | **P0_ACTIVATION_BLOCKER** | IMPLEMENTATION_READY | Publication enqueue after assessment persist; D4/D5 VALIDATED | PKG-01 runtime | Integration | YES | NO | YES | LOW | Runtime PKG-02 |
| `BAT-V2-GAP-LV-PUBLICATION-JOB-CHAIN-001` | Umbrella: not e2e reachable | A | — | CONFIRMED | No automatic LV pub from canonical REST | NONE | CONFIRMED e2e chain broken | **P0_ACTIVATION_BLOCKER** | IMPLEMENTATION_READY | Resolve via PKG-01 + PKG-02 | PKG-01, PKG-02 | E2E test harness | YES | Maybe | YES | MED | Umbrella |
...
| `BAT-V2-GAP-HV-PROVIDER-SOH-LATESTSTATE-TIMESTAMP-001` | VLS SOH without evidence timestamp | D | C | CONFIRMED | VLS-only provider SOH already non-decision-fresh at runtime | UNKNOWN | CONFIRMED: `providerSohObservedAt` from evidence only; VLS-only → `providerSohUsable=false` | **P2** | DECISION_REQUIRED | **RECOMMENDED/PROPOSED:** retain current non-decision-fresh VLS-only semantics (VLS may appear as live/diagnostic; cannot win canonical decision without timestamped evidence) — **pending product/architecture sign-off**. Remaining: production frequency — PRODUCTION_VALIDATION_ONLY / RESEARCH_REQUIRED. If decision-capable VLS SOH required → separate timestamp/evidence-materialization spec (not PKG-04). **Implementation impact option-dependent** — see note below | Independent of PKG-04 winner iteration | Prod frequency query + unit (existing behavior) | MAYBE | MAYBE | MAYBE | N/A | Decision + validation |
...
### Provider VLS SOH — conditional implementation impact

`BAT-V2-GAP-HV-PROVIDER-SOH-LATESTSTATE-TIMESTAMP-001` — master-table **Runtime? / Migration? / Flag? / Rollback** columns are **option-dependent** while readiness remains **DECISION_REQUIRED**. Option not chosen in Phase 4.

| Option | Meaning | Runtime? | Migration? | Flag? | Rollback |
|--------|---------|----------|------------|-------|----------|
| **A** | Retain current non-decision-fresh semantics (**RECOMMENDED/PROPOSED** — pending sign-off) | NO | NO | NO | N/A |
| **B** | Decision-capable VLS SOH — timestamp/evidence materialization **SPEC REQUIRED** | YES | SPEC REQUIRED | TBD | TBD |
