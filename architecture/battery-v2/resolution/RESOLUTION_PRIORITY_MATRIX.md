...
| `BAT-V2-GAP-HV-PROVIDER-SOH-LATESTSTATE-TIMESTAMP-001` | VLS SOH without evidence timestamp | A | F | CONFIRMED | Provider SOH unusable / null | UNKNOWN | CONFIRMED VLS-only path | **P2** | IMPLEMENTATION_READY | Option A: VLS-only never decision-fresh | PKG-04 related | Unit | YES | Maybe | NO | LOW | PKG-04 |
...
| `BAT-V2-GAP-PUB-READINESS-001` | Pub/readiness enablement | C | D | CONFIRMED | Flags default OFF | PARTIAL_POST_CHANGE | CONFIRMED flag wiring | **P2** | NOT_READY | Blocked by LV chain + OQ enablement policy | PKG-01/02 | Staged rollout | YES | NO | YES | MED | Stage 2+ |
