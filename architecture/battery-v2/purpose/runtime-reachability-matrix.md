...
| **HV SOH shadow gate** | ✓ | default OFF | verified ref + cross-session | ✓ shadow pipeline when `HV_CAPACITY_SHADOW` on | ✓ assessments | internal only (`sohGatePassed` may be true) | ✓ `canonical.hv.sohAssessment` | ✗ | **`BATTERY_V2_HV_CAPACITY_SHADOW_ENABLED`** (execution); publication-context: `BATTERY_V2_HV_SOH_PUBLICATION_ENABLED` | CONFIRMED |
| **HV SOH publication-intent** | ✓ (flag exists) | default OFF | n/a — not an execution gate | ✗ no customer publication path | ✗ no HV `battery_publications` | ✗ (`publicationEligible: false` always) | ✗ not summary winner | ✗ | `BATTERY_V2_HV_SOH_PUBLICATION_ENABLED` (reason metadata only) | CONFIRMED |
