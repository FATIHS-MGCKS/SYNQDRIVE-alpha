...
**Snapshot date:** 2026-09-24 (M3.3C C1–C5B on main; M3.3D D0 audit in flight — C5A @ `87599311c` PR #1732; C5B @ `969cc3f19` PR #1733; seal @ `7878aee90` PR #1734; production runtime baseline unchanged `2b0ef15f`)
...
| **M3.3D D0 (architecture)** | Longitudinal profile scientific contract + `M3_3D_LONGITUDINAL_PROFILE_V1` proposal — **audit complete (docs only)** @ `7878aee90` baseline — canonical C3 row authority; `anchorAt` time axis; hybrid persistence recommendation; **no runtime** — `research/M3_3D_D0_LONGITUDINAL_PROFILE_ARCHITECTURE_2026-09-24.md` |
| **M3.3C roadmap (planning)** | **M3.3D** longitudinal profile (**D0 sealed; D1+ engineering next**) · **M3.3E** health/risk/confidence · **M3.3F** production shadow validation · **M3.3G** authoritative cutover · **M3.3H** Vehicle Detail → Health **customer** UI (authoritative outputs only post-G; distinct from C5B) |
| **M3.3C** | **`OPEN`** — C1–C5B **complete on main**; **M3.3D D0 audit**; **D1 implementation pending**; **M3.3E–H pending**; **`BATTERY_V2_REST_SESSION_FEATURES_SHADOW_ENABLED=false`** |
...
| `NEXT_PHASE` | **M3.3D D1** — canonical longitudinal input reader + inclusion policy (after D0 merge); D0 contract @ `M3_3D_D0_LONGITUDINAL_PROFILE_ARCHITECTURE_2026-09-24.md`; **not** health/risk scoring (M3.3E); **no** REST_WAKE cadence promotion; **`BATTERY_V2_REST_SESSION_FEATURES_SHADOW_ENABLED` remains default OFF** until M3.3F authorization |
