...
| **M3.3F (architecture)** | Production shadow materialization + natural-data calibration **gate** — **ARCHITECTURE DRAFT** (not activated; not deployed; not production-ready) — **`M3_3F_REMAINS_PENDING=YES`**; separate C3 vs D3 flags; D3 trigger V1 `ON_DEMAND_INTERNAL_OPS_PLUS_BOUNDED_SCHEDULED_RECONCILIATION`; **`M3_3F_REQUIRES_E3_RUNTIME=NO`**; **`NUMERIC_CALIBRATION_VALUES_SET=NO`** — `research/M3_3F_PRODUCTION_SHADOW_MATERIALIZATION_NATURAL_CALIBRATION_ARCHITECTURE_2026-09-25.md` |
| **M3.3C roadmap (planning)** | **M3.3E E3 COMPLETE ON MAIN**; **M3.3F** architecture → F1 engineering next; **M3.3G–H pending** |
...
| `NEXT_PHASE` | **M3.3F F1 — D3 runtime registration + dedicated materialization flag (default OFF) + observability** — **`M3_3F_REMAINS_PENDING=YES`**; **`D3_RUNTIME_REACHABLE=NO`** until F1+ activation gates; **`D4_RUNTIME_REACHABLE=NO`**; **`E3_RUNTIME_REACHABLE=NO`**; **`PRODUCTION_MATERIALIZATION_READY=NO`**; M3.3G / M3.3H **pending** |
