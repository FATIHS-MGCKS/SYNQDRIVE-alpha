...
**Snapshot date:** 2026-09-26 (M3.3F **F3 C3 shadow runtime activation PASS** @ `F_C3_T0=2026-09-26T11:09:12Z`; production code `2b54a357854c9d44f638ee857f72936967c04992` / release `20260926094359_v4994`; F2 evidence PR #1792 @ `f16ae03c0`; natural C3 validation **PENDING**; graph validated 2026-09-25)
...
| **M3.3D D1 (engineering)** | Bounded canonical longitudinal input reader + inclusion policy (`M3_3D_D1_LONGITUDINAL_INPUT_V1`) — **COMPLETE ON MAIN** squash-merged PR #1737 @ `9577e0f146ddec2b81fc2ede35389a5fa2e6db94`; post-merge seal PR #1738 @ `9d0dbc7d3` — `research/M3_3D_D1_CANONICAL_LONGITUDINAL_INPUT_READER_2026-09-24.md` |
...
| **M3.3E E1 (engineering)** | Pure `buildLongitudinalAssessmentInputV1` adapter — **COMPLETE ON MAIN** merged PR #1765 @ merge `553ba670a0a5b99689c6575417b75d4f0b9fb422` (PR head `4891c6dc48b33fba0a4992158b62566ca3eebcd8`); **`E1_ADAPTER_IMPLEMENTED=YES`**; **`M3_3E_HEALTH_MODEL_IMPLEMENTED=NO`**; 97 unit tests / 3 suites; exact-head CI **28/28 SUCCESS** — `research/M3_3E_E1_LONGITUDINAL_ASSESSMENT_INPUT_ADAPTER_2026-09-25.md` |
| **M3.3E E2 (architecture)** | Longitudinal battery health model scientific contract — **`M3_3E_LONGITUDINAL_HEALTH_EVALUATION_V1`** / **`M3_3E_E2_MODEL_POLICY_V1`** / default calibration **`M3_3E_CALIBRATION_UNSET_V1`** — **COMPLETE ON MAIN** merged PR #1773 @ merge `d7cb2d1e8b6d449752d03cd3733d111fee3cfeeb` (head `8502928af94c6a3d6f000dd41ab413b4ce4c63cc`); E2 post-merge seal merged PR #1775 @ `a58773e72`; **`M3_3E_E2_COMPLETE_ON_MAIN=YES`**; **`M3_3E_E2_POST_MERGE_SEAL=PASS`**; **`M3_3E_E3_IMPLEMENTATION_READY=YES`**; **`M3_3E_HEALTH_MODEL_IMPLEMENTED=NO`**; **`M3_3E_CONCLUSION_BEARING_MODEL_READY=NO`**; **`M3_3F_REMAINS_PENDING=YES`**; `condition=NOT_ASSESSED` only V1 value; no SOH / score / readiness / publication; `CROSS_VERSION_POOLING_DEFAULT=NO`; `NUMERIC_THRESHOLDS_EVIDENCE_BACKED=NO` (11 `CAL-M3.3E-*` items) — `research/M3_3E_E2_LONGITUDINAL_HEALTH_MODEL_ARCHITECTURE_2026-09-25.md` |
| **M3.3E E3 (engineering)** | Pure `evaluateM3_3E_LongitudinalHealthEvaluationV1` — **COMPLETE ON MAIN** merged PR #1778 @ head `984bf2ec3d82eac160c84c0ad8ce2ff796904165` / merge `55fcbe7a229db9858cecc1538b9ec1919ff30fbc`; E3 post-merge seal merged PR #1782 @ `abc2e6c2b`; **`M3_3E_E3_COMPLETE_ON_MAIN=YES`**; **`M3_3E_E3_POST_MERGE_SEAL=PASS`**; **`M3_3E_PURE_LONGITUDINAL_EVALUATOR_IMPLEMENTED=YES`**; E3 golden `4d4b7ee105cb329b6610cde3648733257f46f1cb4b01abc6e519b05206410398`; **`M3_3E_HEALTH_MODEL_IMPLEMENTED=NO`**; **`M3_3E_CONCLUSION_BEARING_MODEL_READY=NO`**; **`E3_RUNTIME_REACHABLE=NO`**; **`E3_PERSISTENCE=NO`** — `research/M3_3E_E3_PURE_LONGITUDINAL_HEALTH_EVALUATOR_2026-09-25.md` |
| **M3.3F (architecture)** | Production shadow materialization + natural-data calibration **gate** — **F0 COMPLETE ON MAIN** merged PR #1784 @ `6d8198fa1` — **`M3_3F_F0_COMPLETE_ON_MAIN=YES`**; **`M3_3F_REMAINS_PENDING=YES`**; separate C3 vs D3 flags; D3 trigger V1 `ON_DEMAND_INTERNAL_OPS_PLUS_BOUNDED_SCHEDULED_RECONCILIATION`; **`M3_3F_REQUIRES_E3_RUNTIME=NO`**; **`NUMERIC_CALIBRATION_VALUES_SET=NO`** — `research/M3_3F_PRODUCTION_SHADOW_MATERIALIZATION_NATURAL_CALIBRATION_ARCHITECTURE_2026-09-25.md` |
| **M3.3F F1 (engineering)** | D3 Nest DI + dedicated materialization flag (default OFF) + gated runtime facade + D3/D4 metric helpers + internal ops CLI — **COMPLETE ON MAIN** (PR #1787); **`M3_3F_F1_COMPLETE_ON_MAIN=YES`**; **`D3_CODE_DI_REGISTERED=YES`**; **`D3_PRODUCTION_WRITES_EFFECTIVELY_GATED_OFF=YES`**; **`PRODUCTION_MATERIALIZATION_READY=NO`** — `research/M3_3F_F1_D3_MATERIALIZATION_RUNTIME_FOUNDATION_2026-09-26.md` |
| **M3.3F F2 (production gate)** | Flag-OFF deploy + zero-write smoke + ops `SKIPPED_FLAG_OFF` — **PASS** @ `2026-09-26`; **`M3_3F_F2_PRODUCTION_GATE=PASS`**; **`F3_ALLOWED=YES`** (pre-F3) — `research/M3_3F_F2_FLAG_OFF_PRODUCTION_DEPLOY_2026-09-26.md` |
| **M3.3F F3 (production activation)** | Controlled C3 shadow **ON** (D3 **OFF**) — **`M3_3F_F3_RUNTIME_ACTIVATION=PASS`** @ `2026-09-26`; **`F_C3_T0=2026-09-26T11:09:12Z`**; **`C3_PRODUCTION_ACTIVATED=YES`**; **`BATTERY_V2_REST_SESSION_FEATURES_SHADOW_ENABLED=true`**; **`F3_NATURAL_C3_VALIDATION=PENDING`**; **`F4_ALLOWED=NO`** — `research/M3_3F_F3_C3_SHADOW_ACTIVATION_2026-09-26.md` |
| **M3.3C roadmap (planning)** | **F3 runtime complete** → **natural C3 evidence follow-up** (read-only); **F4 blocked** until post-T0 row + C5A; **M3.3G–H pending** |
...
| `NEXT_PHASE` | **M3.3F F3 natural C3 evidence follow-up** (read-only) — **`M3_3F_F0_COMPLETE_ON_MAIN=YES`**; **`M3_3F_F1_COMPLETE_ON_MAIN=YES`**; **`M3_3F_F2_PRODUCTION_GATE=PASS`**; **`M3_3F_F3_RUNTIME_ACTIVATION=PASS`**; **`F_C3_T0=2026-09-26T11:09:12Z`**; **`C3_PRODUCTION_ACTIVATED=YES`**; **`F3_NATURAL_C3_VALIDATION=PENDING`**; **`F4_ALLOWED=NO`**; **`D3_PRODUCTION_ACTIVATED=NO`**; **`D3_RUNTIME_REACHABLE=NO`**; **`PRODUCTION_MATERIALIZATION_READY=NO`**; **`M3_3F_REMAINS_PENDING=YES`**; M3.3G / M3.3H **pending** |
