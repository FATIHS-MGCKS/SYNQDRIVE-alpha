...
| **M3.3E E2 (architecture)** | Longitudinal battery health model scientific contract — **`M3_3E_LONGITUDINAL_HEALTH_EVALUATION_V1`** / **`M3_3E_E2_MODEL_POLICY_V1`** / default calibration **`M3_3E_CALIBRATION_UNSET_V1`** — **M3.3E E2 ARCHITECTURE COMPLETE** (docs-only draft PR; merge + seal pending); **`M3_3E_HEALTH_MODEL_IMPLEMENTED=NO`**; **`M3_3E_E3_IMPLEMENTATION_READY=YES`** (pure, non-persisted, fail-closed evaluator only); **`M3_3E_CONCLUSION_BEARING_MODEL_READY=NO`**; `condition=NOT_ASSESSED` only V1 value; no SOH / score / readiness / publication; `CROSS_VERSION_POOLING_DEFAULT=NO`; `NUMERIC_THRESHOLDS_EVIDENCE_BACKED=NO` (11 `CAL-M3.3E-*` items) — `research/M3_3E_E2_LONGITUDINAL_HEALTH_MODEL_ARCHITECTURE_2026-09-25.md` |
| **M3.3C roadmap (planning)** | **M3.3E E3** pure fail-closed longitudinal evaluator engineering next (after E2 merge + seal) — **M3.3F–H** pending |
| **M3.3C** | **`OPEN`** — C1–C5B + **M3.3D + M3.3E E0–E0.2 + M3.3E E1 on main**; **M3.3E E2 architecture (draft PR)**; **M3.3F–H pending**; **`BATTERY_V2_REST_SESSION_FEATURES_SHADOW_ENABLED=false`** |
...
| `NEXT_PHASE` | **M3.3E E3 PURE LONGITUDINAL EVALUATOR** (after E2 merge + post-merge seal) — implements `M3_3E_LONGITUDINAL_HEALTH_EVALUATION_V1` under `M3_3E_CALIBRATION_UNSET_V1` (descriptive statistics + first-class no-conclusion; no condition, no persistence); E2 scientific contract **ARCHITECTURE COMPLETE** (draft PR); E1 deterministic **assessment input** **COMPLETE ON MAIN** PR #1765; **not** production D3 materialization (**M3.3F** gate remains separate); **`M3_3F_REMAINS_PENDING=YES`**; **`D3_RUNTIME_REACHABLE=NO`**; **`D4_RUNTIME_REACHABLE=NO`**; **`PRODUCTION_MATERIALIZATION_READY=NO`**; M3.3G / M3.3H **pending** |
