...
| **B1.2Y1 implementation (PR open)** | **`PROVIDER_OBSERVABILITY_GAP` runtime foundation** — schema + service + poll hook + tests; **`BATTERY_V2_PROVIDER_OBSERVABILITY_GAP_ENABLED` default OFF** — production behavior unchanged until authorized activation |
