...
| **M3.3C roadmap (planning)** | **M3.3E E2 HEALTH MODEL ARCHITECTURE** next — **M3.3F–H** pending |
| **M3.3C** | **`OPEN`** — C1–C5B + **M3.3D + M3.3E E0–E0.2 + M3.3E E1 on main**; **M3.3F–H pending**; **`BATTERY_V2_REST_SESSION_FEATURES_SHADOW_ENABLED=false`** |
...
| `NEXT_PHASE` | **M3.3E E2 HEALTH MODEL ARCHITECTURE** — scientific policy for longitudinal battery condition (E1 deterministic **assessment input** **COMPLETE ON MAIN** PR #1765); **not** production D3 materialization (**M3.3F** gate remains separate); **`M3_3F_REMAINS_PENDING=YES`**; **`D3_RUNTIME_REACHABLE=NO`**; **`D4_RUNTIME_REACHABLE=NO`**; **`PRODUCTION_MATERIALIZATION_READY=NO`**; M3.3G / M3.3H **pending** |
