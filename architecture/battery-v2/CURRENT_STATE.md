...
| M3.2B shadow shutdown evidence (`2026-09-07`, PR #1560) | **DEPLOYED** @ `0ba96e03` release `20260907204434_v4994`; Phase B **PASS** flag-off; Phase C **PASS** shadow ON @ **`M3_2B_PHASE_C_T0=2026-09-07T22:47:14Z`**; `SHADOW_FLAG_EFFECTIVE=true`; first natural probe @ `2026-09-08T05:10:44Z`: KS MX + KS MS post-T0 trips found but **not finalized** (`trip_status=ONGOING`); **0 shadow rows** (expected until finalize); `OVERALL_RESULT=INSUFFICIENT_EVIDENCE`; see Phase B/C docs + `research/M3_2B_FIRST_NATURAL_SHUTDOWN_EVIDENCE_PROBE_2026-09-08.md` |
| `NEXT_PHASE` | **Re-probe after trip finalization** (`trip_status=COMPLETED`) on post-T0 KS MX / KS MS trips → shadow context + observation forensic review; `M3_2C_ALLOWED=NO`; `NEXT_VALIDATION_TRIGGER=NATURAL_POST_ACTIVATION_TRIP_SHUTDOWN_FINALIZED` |
