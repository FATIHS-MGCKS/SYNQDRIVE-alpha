...
- **PKG-01:** **`IMPLEMENTED`** (runtime, 2026-09-02) — canonical LV REST → assessment handoff per D1/D2/D3. `IMPLEMENTED` ≠ `PRODUCTION_VALIDATED` ≠ activation-ready ≠ cutover-ready. Legacy `reconcilePendingAssessments` (`batteryFeatures`) retained; canonical repair via `reconcileCanonicalRestAssessmentHandoffs`. No `BATTERY_V2_LV_HANDOFF_ENABLED`; REST_SHADOW untouched; M4 not authorized.
