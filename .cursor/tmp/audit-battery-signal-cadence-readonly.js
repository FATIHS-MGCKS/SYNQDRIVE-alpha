/* READ-ONLY one-off analysis - not part of repo */
const fs = require('fs');
...
(async () => {
  ...
  const out = { generatedAt: new Date().toISOString(), periodDays: DAYS, since: since.toISOString() };
  ...
  out.pollLogRange = await p.dimoPollLog.aggregate({
    where: { jobType: 'SNAPSHOT', createdAt: { gte: since } },
  ...
  for (const v of vehicles) {
    const logs = await p.dimoPollLog.findMany({
      where: { vehicleId: v.id, jobType: 'SNAPSHOT', createdAt: { gte: since } },
      orderBy: { startedAt: 'asc' },
      select: { startedAt: true, status: true, finishedAt: true },
    });
    const gaps = [];
    const durations = [];
    for (let i = 1; i < logs.length; i++) gaps.push((logs[i].startedAt - logs[i - 1].startedAt) / 1000);
    for (const l of logs) if (l.finishedAt) durations.push(l.finishedAt - l.startedAt);
    pollIntervals[v.id] = {
      ...
      durationMedianMs: median(durations), durationP95Ms: pct(durations, 0.95),
    };
  }
  ...
  } catch (e) { out.chRange = { error: e.message }; }

  let chVehicles = [];
  try {
    chVehicles = await chQuery(`SELECT vehicle_id, count() FROM telemetry_snapshots WHERE recorded_at >= now() - INTERVAL ${DAYS} DAY GROUP BY vehicle_id ORDER BY count() DESC FORMAT TabSeparated`);
  } catch (e) { out.chVehicleQueryError = e.message; }
  ...
  for (const [vehicleId] of chVehicles) {
    ...
    const gaps = [];
    let repeatRec = 0;
    const uniqueRec = new Set();
    let socRepeat = 0, prevSoc = null, maxSocStreak = 0, streak = 0;
    const freshnessBuckets = { gt30s: 0, gt2m: 0, gt5m: 0, gt30m: 0, gt1h: 0 };

    for (let i = 0; i < rows.length; i++) {
      uniqueRec.add(rows[i][0]);
      if (i > 0) {
        const g = (new Date(rows[i][0] + 'Z') - new Date(rows[i - 1][0] + 'Z')) / 1000;
        gaps.push(g);
        if (rows[i][0] === rows[i - 1][0]) repeatRec++;
        // provider timestamp age proxy: if gap between polls is small but recorded_at unchanged => stale
        if (g <= 120 && rows[i][0] === rows[i - 1][0]) {
          if (g > 30) freshnessBuckets.gt30s++;
          if (g > 120) freshnessBuckets.gt2m++;
        }
      }
  ...
  out.pollVsPersist = Object.fromEntries(vehicles.map((v) => {
    const pi = pollIntervals[v.id];
    const ch = chPerVehicle[v.id];
  ...
  out.hvSnapshots = await p.$queryRaw`
    SELECT vehicle_id, COUNT(*)::bigint cnt, COUNT(DISTINCT recorded_at)::bigint unique_rec,
      MIN(recorded_at) min_at, MAX(recorded_at) max_at
    FROM hv_battery_health_snapshots WHERE recorded_at >= ${since} GROUP BY vehicle_id`;
  ...
  for (const v of vehicles.filter((x) => ['ELECTRIC', 'PLUGIN_HYBRID'].includes(x.fuelType))) {
    ...
    for (let i = 1; i < snaps.length; i++) {
      gaps.push((snaps[i].recordedAt - snaps[i - 1].recordedAt) / 1000);
      if (snaps[i].socPercent === snaps[i - 1].socPercent) sameSoc++;
    }
    hvGaps[v.id] = { label: label(v), rows: snaps.length, gapMedianSec: median(gaps), gapP95Sec: pct(gaps, 0.95), gapMaxSec: gaps.length ? Math.max(...gaps) : null, sameSocRows: sameSoc, uniqueSoc: new Set(snaps.map((s) => s.socPercent)).size };
  }
  out.hvGaps = hvGaps;
  ...
  if (ks) {
    ...
    const fetchMinusSourceMs = vls?.sourceTimestamp && vls?.providerFetchedAt ? vls.providerFetchedAt - vls.sourceTimestamp : null;
    out.ksFh660e = { fuelType: ks.fuelType, state: det?.state, lastSeenAt: vls?.lastSeenAt, sourceTimestamp: vls?.sourceTimestamp, providerFetchedAt: vls?.providerFetchedAt, fetchMinusSourceMs, lv: vls?.lvBatteryVoltage, evSoc: vls?.evSoc, speed: vls?.speedKmh, ignition: vls?.isIgnitionOn };
    const pi = pollIntervals[ks.id];
    out.ksFh660ePolls = pi;
    out.ksFh660eCh = chPerVehicle[ks.id];
  ...
  const allGaps = Object.values(pollIntervals).map((x) => x.gapMedianSec).filter((x) => x != null);
  out.fleetPollGapMedianSec = median(allGaps);
  const allChGaps = Object.values(chPerVehicle).map((x) => x.gapMedianSec).filter((x) => x != null);
  out.fleetChRecordedGapMedianSec = median(allChGaps);
  const repeatPcts = Object.values(chPerVehicle).map((x) => x.repeatRecordedAtPct);
  out.fleetRepeatRecordedAtMedianPct = median(repeatPcts);
