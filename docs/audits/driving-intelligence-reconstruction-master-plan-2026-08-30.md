...
**Status:** **DONE** (2026-08-31) — manifest v1.0.0 frozen · Phase 3A **READY / UNGATED_TO_START**

**Gate:** Completion of **2F.1** ungates **Phase 3A only** — **satisfied 2026-08-31** — not 3B/3C/3D.
...
**Phase 2 overall:** IN_PROGRESS (2A+2B+2C+2D.0+2D+2E+2F+**2F.1** done; **3A READY**; 2G–2I not started). Validation uses **profile-scoped gates** (§1.6.8a).
...
- **Phase 3A:** **READY / UNGATED_TO_START** — 2F.1 manifest v1.0.0 frozen 2026-08-31
...
- **Frozen manifest:** `DIMO_LTE_R1_REFERENCE_MANIFEST` v**1.0.0** · `CAN-33-2026-08-31`
- **Minimum Pareto:** **17** canonical keys + supplemental lat/lon/ignition/lastSeen + T4 native events
- **Physics conditional:** yaw, wheel speeds, brake hydraulics — 0/4 on audit fleet but **must attempt capture** when capable
- **Timestamp gap (CONFIRMED_FROM_CODE):** `synqReceivedAt` required in FR contract — current HF path incomplete
- **PHEV/BEV GT:** `PENDING_REFERENCE_VEHICLE` — overlays frozen, no audit reference vehicle
- **Phase 3A:** **READY / UNGATED_TO_START** — Flight Recorder implementation not started
...
**PRIMARY** calibration/reference workstream · **`READY / UNGATED_TO_START`** (not started)

**May start when:** the **`DIMO_LTE_R1`** target-profile manifest (Phase **2F.1**) is frozen. **Condition satisfied 2026-08-31.**
...
- **`DIMO_LTE_R1` reference manifest v1.0.0 (2F.1)** ✓
...
**Status:** **NOT_STARTED — `READY / UNGATED_TO_START`** (2F.1 manifest frozen 2026-08-31; Flight Recorder not implemented)
...
| Phase 2F.1 DIMO LTE_R1 reference manifest | DONE | `dimo-phase-2f1-lte-r1-reference-manifest-2026-08-31.md` + JSON v1.0.0 — ungates 3A |
...
| Phase 3A DIMO LTE_R1 reference program | NOT_STARTED | **READY / UNGATED_TO_START** (2F.1 manifest frozen) |
...
10. ~~**Execute Phase 2F.1:** `DIMO_LTE_R1` reference manifest.~~ **Done** — manifest v1.0.0 frozen; Phase 3A ungated.
11. **Execute Phase 3A** (LTE R1) — Flight Recorder / reference program using frozen manifest. **Does not wait** for steps 12–14.
