...
- Draft-PR: **Evaluations Recovery E8 – Predictive Risk & estimatedExposure** (OPEN, Draft, base `main`)
...
| **E8A** | **Predictive risk + estimatedExposure authority / model governance freeze** | ✅ (dieser Branch, docs-only) |
| E8B | Canonical backend predictive risk + offline validation | ⬜ offen |
...
|-----------|--------------|
| E8 predictive target | `FLEET_UNPLANNED_MAINTENANCE_DISRUPTION` (org/station scope, horizon `NEXT_30_DAYS`) |
...
E8A froze predictive risk for FLEET_UNPLANNED_MAINTENANCE_DISRUPTION (NEXT_30_DAYS) without numeric probability until offline calibration. estimatedExposure remains DEFERRED. Nächstes Gate: E8B canonical backend. Kein E9 Forecast UI. PR bleibt Draft.
```
