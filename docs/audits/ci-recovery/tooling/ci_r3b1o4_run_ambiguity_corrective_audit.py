#!/usr/bin/env python3
"""CI-R3B1O.4 ambiguity corrective catalog authority acceptance orchestrator."""
...
from ci_r3b1o4_constants import AMBIGUITY_CORRECTIVE_STRATEGY_DB_PREFIX, R3B1O3_BRANCH, ensure_r3b1o4_workdir
...
from ci_r3b1o4_test_source_hashes import (
    ...
    write_ambiguity_corrective_test_source_hash_manifest,
)
...
AMBIGUITY_CORRECTIVE_INPUTS = [
    "ci-r3b1o4-binding-corrective-expected-catalog-deltas-2026-08.json",
    "ci-r3b1o4-binding-corrective-authority-candidate-inventory-2026-08.json",
    "ci-r3b1o4-binding-corrective-full-catalog-authority-2026-08.json",
    "ci-r3b1o4-binding-corrective-delta-authority-proof-2026-08.json",
    "ci-r3b1o4-binding-corrective-statement-crossvalidation-2026-08.json",
    "ci-r3b1o4-binding-corrective-golden-tests-2026-08.json",
    "ci-r3b1o4-binding-corrective-final-acceptance-summary-2026-08.json",
...
def build_ambiguity_corrective_input_manifest(baseline: dict) -> dict:
    tooling = Path(__file__).resolve().parent
    bound = {}
    for rel in AMBIGUITY_CORRECTIVE_INPUTS:
    ...
    for name in TOOLING_INPUTS:
        path = tooling / name
        bound[f"tooling/{name}"] = {"sha256": sha256_file(path) if path.exists() else None, "exists": path.exists()}
    return {"schema_version": 1, "phase": "CI-R3B1O.4-ambiguity-corrective", "baseline": baseline, "bound_inputs": bound}


def build_authority_decision_contract() -> dict:
    return {
        "schema_version": 1,
        "phase": "CI-R3B1O.4-ambiguity-corrective",
        "decision_model": {
            "semantic_match_count_0": "UNAUTHORIZED_FINAL_DELTA",
            "semantic_match_count_1": "unique authorized candidate",
            "semantic_match_count_gt_1": "AMBIGUOUS_DELTA_AUTHORITY",
        },
        "forbidden_disambiguation": [
            "migration timestamp",
            "migration position",
            "recovery/canonical classification",
            "M252 classification",
            "match mode",
            "statement ordinal",
            "source priority",
...
def build_m252_provenance_contract(expected: dict) -> dict:
    return {
        "schema_version": 1,
        "phase": "CI-R3B1O.4-ambiguity-corrective",
        "creator_provenance": "real SQL statements only via _effects_from_sql per statement ordinal",
        "canonical_parity_authority": expected.get("canonical_parity_authority", {}),
        "synthetic_m252_creator_count": expected.get("synthetic_m252_creator_count", 0),
        "pass": expected.get("synthetic_m252_creator_count", 0) == 0,
    }


def build_statement_provenance_artifact(*, expected: dict, catalog_delta: dict, execution_set: dict) -> dict:
    proofs = catalog_delta.get("proofs", [])
    missing_ordinal = sum(1 for p in proofs if p.get("statement_ordinal") is None)
    missing_sha = sum(1 for p in proofs if not p.get("statement_sha256"))
    return {
        "schema_version": 1,
        "phase": "CI-R3B1O.4-ambiguity-corrective",
        "executing_migration_count": execution_set.get("executing_migration_count"),
        "expected_effect_count": expected.get("expected_effect_count"),
        "statement_ordinal_null_count": expected.get("statement_ordinal_null_count"),
        "statement_sha_mismatch_count": expected.get("statement_sha_mismatch_count"),
        "synthetic_m252_creator_count": expected.get("synthetic_m252_creator_count"),
        "authorized_missing_statement_ordinal": missing_ordinal,
        "authorized_missing_statement_sha": missing_sha,
        "pass": missing_ordinal == 0 and missing_sha == 0 and expected.get("synthetic_m252_creator_count", 0) == 0,
    }


def main() -> int:
    strict_empty, _wt_text = strict_worktree_empty()
    binding_summary = DATA / "ci-r3b1o4-binding-corrective-final-acceptance-summary-2026-08.json"
    baseline = {
        ...
        "BINDING_CORRECTIVE_SUMMARY_SHA256": sha256_file(binding_summary) if binding_summary.exists() else None,
    }
    if not strict_empty:
        write_json(
            DATA / f"{PREFIX}-final-acceptance-summary-2026-08.json",
            {"final_status": "CI_R3B1O4_CORRECTIVE_BASELINE_NOT_CLEAN", "pass": False, "baseline": baseline},
        )
        print(json.dumps({"final_status": "CI_R3B1O4_CORRECTIVE_BASELINE_NOT_CLEAN", "pass": False}, indent=2))
        return 1

    pre_hashes = build_ambiguity_corrective_test_source_hash_manifest()
    write_ambiguity_corrective_test_source_hash_manifest(pre_hashes)
    write_json(DATA / f"{PREFIX}-input-manifest-2026-08.json", build_ambiguity_corrective_input_manifest(baseline))
    write_json(DATA / f"{PREFIX}-authority-decision-contract-2026-08.json", build_authority_decision_contract())
    ...
    write_json(
        DATA / f"{PREFIX}-expected-catalog-effects-2026-08.json",
        {
            "schema_version": 1,
            "phase": "CI-R3B1O.4-ambiguity-corrective",
            "creator_effects": expected.get("creator_effects", expected.get("effects", [])),
            "canonical_parity_authority": expected.get("canonical_parity_authority"),
            "synthetic_m252_creator_count": expected.get("synthetic_m252_creator_count"),
            "pass": expected.get("pass"),
        },
    )
    write_json(DATA / f"{PREFIX}-m252-provenance-contract-2026-08.json", build_m252_provenance_contract(expected))
    implicit = build_implicit_catalog_effects(expected=expected)
    write_json(DATA / f"{PREFIX}-implicit-effects-2026-08.json", implicit)
    ...
    strategy = run_tail_reconciliation_strategy(
        ...
        strategy_id=AMBIGUITY_CORRECTIVE_STRATEGY_DB_PREFIX,
    )
    ...
    post_hashes = build_ambiguity_corrective_test_source_hash_manifest()
    ...
    crossvalidation["phase"] = "CI-R3B1O.4-ambiguity-corrective"
    write_json(DATA / f"{PREFIX}-evidence-code-crossvalidation-2026-08.json", crossvalidation)
    write_ambiguity_corrective_test_source_hash_manifest(post_hashes, pre_hashes=pre_hashes)
    hash_payload = json.loads((DATA / f"{PREFIX}-source-hash-manifest-2026-08.json").read_text())
    no_ranking = build_no_ranking_proof()
    write_json(DATA / f"{PREFIX}-no-ranking-proof-2026-08.json", no_ranking)
    ...
    write_json(DATA / f"{PREFIX}-full-catalog-authority-2026-08.json", catalog_delta)
    write_json(
        DATA / f"{PREFIX}-authority-candidate-inventory-2026-08.json",
        {
            "schema_version": 1,
            "phase": "CI-R3B1O.4-ambiguity-corrective",
            "candidate_count": len(catalog_delta.get("candidate_inventory", [])),
            "rows": catalog_delta.get("candidate_inventory", []),
        },
    )
    proof_payload = {
        "schema_version": 1,
        ...
        "authorized_missing_statement_ordinal": sum(1 for p in catalog_delta.get("proofs", []) if p.get("statement_ordinal") is None),
        "authorized_missing_statement_sha": sum(1 for p in catalog_delta.get("proofs", []) if not p.get("statement_sha256")),
        "pass": catalog_delta.get("pass", False),
        "proofs": catalog_delta.get("proofs", []),
    }
    write_json(DATA / f"{PREFIX}-delta-authority-proof-2026-08.json", proof_payload)
    write_json(
        DATA / f"{PREFIX}-final-candidate-inventory-2026-08.json",
        {
            "schema_version": 1,
            "phase": "CI-R3B1O.4-ambiguity-corrective",
            "candidate_count": len(catalog_delta.get("candidate_inventory", [])),
    ...
    write_json(
        DATA / f"{PREFIX}-statement-provenance-2026-08.json",
        build_statement_provenance_artifact(expected=expected, catalog_delta=catalog_delta, execution_set=execution_set),
    )
    ...
    catalog_engine["phase"] = "CI-R3B1O.4-ambiguity-corrective"
    write_catalog_engine_crossvalidation(catalog_engine, prefix=PREFIX)
    ...
    terminal = evaluate_ambiguity_corrective_terminal_acceptance(
        worktree_strict_empty=strict_empty,
        ...
        statement_crossvalidation_pass=statement_xval["pass"],
        ...
        authority_statement_unbound=catalog_delta["counts"]["AUTHORITY_STATEMENT_UNBOUND"],
        key_only_authorization=catalog_delta["counts"]["key_only_authorization"],
        ...
        no_ranking_proof_pass=no_ranking.get("pass", False),
        synthetic_m252_creator_count=expected.get("synthetic_m252_creator_count", 0),
    )
    ...
    summary = {
        ...
        "statement_crossvalidation": {
            "missing_statement": statement_xval.get("missing_statement"),
            "sha_mismatch": statement_xval.get("sha_mismatch"),
            "family_mismatch": statement_xval.get("family_mismatch"),
            "pass": statement_xval.get("pass"),
        },
        ...
        "source_hash_manifest": {"pre_post_match": hash_payload.get("pre_post_match"), "pass": hash_payload.get("pass")},
    ...
    subprocess.run([sys.executable, str(Path(__file__).with_name("ci_r3b1o4_generate_report.py")), "--ambiguity-corrective"], cwd=Path(__file__).parent)
