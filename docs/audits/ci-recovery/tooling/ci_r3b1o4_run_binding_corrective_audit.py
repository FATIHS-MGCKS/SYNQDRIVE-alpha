#!/usr/bin/env python3
"""CI-R3B1O.4 binding corrective semantic catalog authority acceptance orchestrator."""
...
from ci_r3b1o3_m252_complete_physical_authority import build_m252_complete_physical_authority
...
from ci_r3b1o4_constants import BINDING_CORRECTIVE_STRATEGY_DB_PREFIX, R3B1O3_BRANCH, ensure_r3b1o4_workdir
...
from ci_r3b1o4_terminal_gate import evaluate_binding_corrective_terminal_acceptance
from ci_r3b1o4_test_source_hashes import (
    build_binding_corrective_test_source_hash_manifest,
    write_binding_corrective_test_source_hash_manifest,
)
...
BINDING_CORRECTIVE_INPUTS = [
    "ci-r3b1o4-final-corrective-execution-set-2026-08.json",
    "ci-r3b1o4-final-corrective-expected-catalog-deltas-2026-08.json",
    "ci-r3b1o4-final-corrective-implicit-catalog-effects-2026-08.json",
    "ci-r3b1o4-final-corrective-raw-catalog-deltas-2026-08.json",
    "ci-r3b1o4-final-corrective-full-catalog-delta-authority-2026-08.json",
    "ci-r3b1o4-final-corrective-delta-authority-proof-2026-08.json",
    "ci-r3b1o4-final-corrective-acceptance-summary-2026-08.json",
    "ci-r3b1o4-final-corrective-final-acceptance-summary-2026-08.json",
    "ci-r3b1o4-final-corrective-golden-tests-2026-08.json",
...
def build_binding_corrective_input_manifest(baseline: dict) -> dict:
    tooling = Path(__file__).resolve().parent
    bound = {}
    for rel in BINDING_CORRECTIVE_INPUTS:
    ...
    return {"schema_version": 1, "phase": "CI-R3B1O.4-binding-corrective", "baseline": baseline, "bound_inputs": bound}


def main() -> int:
    strict_empty, _wt_text = strict_worktree_empty()
    final_summary = DATA / "ci-r3b1o4-final-corrective-final-acceptance-summary-2026-08.json"
    baseline = {
        ...
        "FINAL_CORRECTIVE_SUMMARY_SHA256": sha256_file(final_summary) if final_summary.exists() else None,
    }
    if not strict_empty:
        write_json(
    ...
    pre_hashes = build_binding_corrective_test_source_hash_manifest()
    write_binding_corrective_test_source_hash_manifest(pre_hashes)
    write_json(DATA / f"{PREFIX}-input-manifest-2026-08.json", build_binding_corrective_input_manifest(baseline))
    ...
    strategy = run_tail_reconciliation_strategy(
        ...
        strategy_id=BINDING_CORRECTIVE_STRATEGY_DB_PREFIX,
    )
    ...
    golden_run = subprocess.run(
        [sys.executable, str(Path(__file__).with_name("ci_r3b1o4_golden_tests.py")), "--binding-corrective"],
    ...
    post_hashes = build_binding_corrective_test_source_hash_manifest()
    ...
    crossvalidation["phase"] = "CI-R3B1O.4-binding-corrective"
    write_json(DATA / f"{PREFIX}-evidence-code-crossvalidation-2026-08.json", crossvalidation)
    write_binding_corrective_test_source_hash_manifest(post_hashes, pre_hashes=pre_hashes)
    ...
    catalog_engine["phase"] = "CI-R3B1O.4-binding-corrective"
    ...
    terminal = evaluate_binding_corrective_terminal_acceptance(
        worktree_strict_empty=strict_empty,
    ...
    subprocess.run([sys.executable, str(Path(__file__).with_name("ci_r3b1o4_generate_report.py")), "--binding-corrective"], cwd=Path(__file__).parent)
