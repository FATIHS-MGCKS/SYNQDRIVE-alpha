#!/usr/bin/env python3
"""Capture pre-157 PostgreSQL catalog snapshot for tire lifecycle tables."""
...
from ci_r3b1f_constants import DATA, PRE157_BOUNDARY, TIRE_PROPERTIES
from replay_evidence_lib import PgConfig, psql

OUT = DATA / "ci-r3b1f-pre-157-catalog-snapshot-2026-08.json"
...
def main() -> int:
    ...
    for table in TABLES:
        ...
        table_indexes = {
            name: idx
            for name, idx in raw["indexes"].items()
            if idx.get("table") == table
        }
        table_fks = {
            name: fk
            for name, fk in raw.get("foreign_keys", {}).items()
            if fk.get("local_table") == table
        }
        table_pks = {
            name: pk
            for name, pk in raw.get("primary_keys", {}).items()
            if pk.get("table") == table
        }
