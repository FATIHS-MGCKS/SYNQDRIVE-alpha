#!/usr/bin/env python3
"""Build exact predecessor contracts for genuinely remaining gaps (CI-R3B1F.1.1)."""
...
OUT = DATA / "ci-r3b1f111-exact-predecessor-contracts-2026-08.json"
VALIDATION = DATA / "ci-r3b1f111-contract-validation-summary-2026-08.json"
TOPOLOGY = DATA / "ci-r3b1f111-repair-topology-2026-08.json"
...
def build_tire_status_contract(schema_sha256: str) -> dict:
    return {
        "contract_id": "R3B1F111-vehicle_tire_setups-status",
        ...
        "repair_boundary": {
            ...
            "topology_id": "R3B1F111-SLOT13-EXT-STATUS",
            "rationale": (
        ...
        "provenance": {
            "sources": [
                ...
                "catalog:ci-r3b1f111-pre-157-catalog-snapshot:vehicle_tire_setups.status=missing",
                "replay:ci-r3b1f111-pre157-replay-state:last_applied=slot13",
            ]
        },
        "enum_prerequisite": {
    ...
    for gap in gaps:
        if gap["relation"] == "vehicle_tire_setups" and gap["property"] == "status":
            ...
            topology_slots.append(
                {
                    "repair_id": "R3B1F111-SLOT13-EXT-STATUS",
                    "after_migration": PRE157_BOUNDARY,
                    "before_migration": TIRE_CONSUMER,
