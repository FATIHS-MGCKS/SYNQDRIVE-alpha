#!/usr/bin/env python3
"""Creator-state regression fixtures and analyzer chronology tests (CI-R3B1F.1.1)."""
...
class CreatorParserTests(unittest.TestCase):
    ...
    def test_fixture_b_create_table_columns(self) -> None:
        ...
        """
        table, cols, constraints, parsed = parse_create_table_statement(stmt)
        self.assertEqual(table, "sample_table")
        self.assertEqual([c.name for c in cols], ["id", "status", "created_at"])
        self.assertEqual(constraints, ["sample_table_pkey"])
        pk = next(c for c in parsed if c.name == "sample_table_pkey")
