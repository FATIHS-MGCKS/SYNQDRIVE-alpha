#!/usr/bin/env python3
"""Immutability audit since R3B1H.1 base (CI-R3B1H.1.1)."""
from __future__ import annotations

import hashlib
import json
import subprocess
from pathlib import Path

from ci_r3b1h111_constants import BASE_R3B1H1_SHA, DATA, MIG_ROOT, PRE249_BOUNDARY, REPO

OUT = DATA / "ci-r3b1h111-immutability-audit-2026-08.json"
R3B1G_REPAIR = "20260716182730_ci_r3b_tire_setup_status_predecessor"
MIG_157 = "20260716183000_tire_lifecycle_invariants"


def migration_dirs_at(rev: str) -> list[str]:
    out = subprocess.check_output(["git", "ls-tree", "-d", "--name-only", rev, "backend/prisma/migrations/"], cwd=REPO, text=True)
    return sorted(line.strip().split("/")[-1] for line in out.splitlines() if line.strip())


def migration_sql_at(rev: str, mig: str) -> str | None:
    path = f"backend/prisma/migrations/{mig}/migration.sql"
    try:
        return subprocess.check_output(["git", "show", f"{rev}:{path}"], cwd=REPO, text=True)
    except subprocess.CalledProcessError:
        return None


def main() -> int:
    baseline_dirs = migration_dirs_at(BASE_R3B1H1_SHA)
    current_dirs = sorted(p.name for p in MIG_ROOT.iterdir() if p.is_dir())
    new_dirs = [d for d in current_dirs if d not in baseline_dirs]
    migration_changes = []
    for mig in baseline_dirs:
        base_sql = migration_sql_at(BASE_R3B1H1_SHA, mig)
        cur_path = MIG_ROOT / mig / "migration.sql"
        if base_sql is None or not cur_path.is_file() or cur_path.read_text() != base_sql:
            migration_changes.append({"migration": mig})

    diff_names = subprocess.run(["git", "diff", "--name-only", BASE_R3B1H1_SHA], cwd=REPO, capture_output=True, text=True).stdout.splitlines()
    allowed = ("docs/audits/ci-recovery/", ".gitignore")
    forbidden = [p for p in diff_names if p.strip() and not any(p.startswith(a) for a in allowed)]

    out = {
        "schema_version": 1,
        "phase": "CI-R3B1H.1.1",
        "baseline_sha": BASE_R3B1H1_SHA,
        "preexisting_migration_sql_modified": len(migration_changes),
        "new_prisma_migration_directories": len(new_dirs),
        "r3b1g_tire_repair_changed": any(c.get("migration") == R3B1G_REPAIR for c in migration_changes),
        "migration_157_changed": any(c.get("migration") == MIG_157 for c in migration_changes),
        "migration_249_changed": any(c.get("migration") == PRE249_BOUNDARY for c in migration_changes),
        "schema_prisma_changed": "backend/prisma/schema.prisma" in diff_names,
        "runtime_code_changed": any(p.startswith(("backend/src/", "frontend/")) for p in diff_names),
        "forbidden_non_audit_changes": forbidden,
        "pass": len(migration_changes) == 0 and len(new_dirs) == 0 and not forbidden,
    }
    OUT.write_text(json.dumps(out, indent=2) + "\n")
    print(json.dumps({"pass": out["pass"], "modified": out["preexisting_migration_sql_modified"]}, indent=2))
    return 0 if out["pass"] else 1


if __name__ == "__main__":
    raise SystemExit(main())
