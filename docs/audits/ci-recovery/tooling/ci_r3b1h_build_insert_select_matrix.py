#!/usr/bin/env python3
"""Build INSERT-SELECT dependency matrix for migrations 249→HEAD (CI-R3B1H)."""
...
from ci_r3b1h_constants import DATA, FIRST_SCANNED, INSERT_SELECT_GAP_CONTEXTS, REPO as REPO2
...
OUT = DATA / "ci-r3b1h-insert-select-dependency-matrix-2026-08.json"
...
def unique_actionable(records: list[dict], table_creators: set[str]) -> list[dict]:
    ...
    for r in records:
        ...
        if r.get("required_object_type") != "column":
            continue
        if r.get("scope_kind") == "TARGET_COLUMN" and r.get("dependency_context") == "INSERT_SELECT_TARGET":
            continue
...
def build_matrix() -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "phase": "CI-R3B1H",
        "insert_select_analyzer_hardened": True,
