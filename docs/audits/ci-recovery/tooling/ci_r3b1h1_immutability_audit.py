#!/usr/bin/env python3
"""Immutability audit for CI-R3B1H.1 — no migration/schema/runtime changes since R3B1H."""
...
from ci_r3b1h1_constants import BASE_R3B1H_SHA, DATA, MIG_ROOT, PRE249_BOUNDARY, REPO

OUT = DATA / "ci-r3b1h1-immutability-audit-2026-08.json"
...
def main() -> int:
    baseline_dirs = migration_dirs_at(BASE_R3B1H_SHA)
    ...
    for mig in baseline_dirs:
        base_sql = migration_sql_at(BASE_R3B1H_SHA, mig)
        ...
        if base_sql is None or not cur_path.is_file():
            ...
            continue
        if cur_path.read_text() != base_sql:
            migration_changes.append({"migration": mig})

    diff_names = subprocess.run(
        ["git", "diff", "--name-only", BASE_R3B1H_SHA],
        cwd=REPO,
        capture_output=True,
        text=True,
    ).stdout.splitlines()
    allowed_prefix = "docs/audits/ci-recovery/"
    forbidden = [p for p in diff_names if p.strip() and not p.startswith(allowed_prefix)]

    out = {
        "schema_version": 1,
        "phase": "CI-R3B1H.1",
        "baseline_sha": BASE_R3B1H_SHA,
