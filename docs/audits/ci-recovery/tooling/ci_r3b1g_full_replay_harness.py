#!/usr/bin/env python3
"""CI-R3B1G complete fresh migration replay harness."""
...
from ci_r3b1g_constants import (
    BASE_R3B1F111_SHA,
    DATA,
...
RESULT_PATH = DATA / "ci-r3b1g-full-fresh-replay-result-2026-08.json"
AUTH_MANIFEST = DATA / "ci-r3b1g-implementation-authority-manifest-2026-08.json"
REPLAY_MANIFEST = DATA / "ci-r3b1g-replay-input-manifest-2026-08.json"
...
def classify_failure(failed: str | None) -> str:
    ...
    if failed == R3B1G_REPAIR_MIGRATION:
        return "R3B1G_REPAIR_DEFECT"
    if failed == TIRE_CONSUMER:
        return "R3B1G_REPAIR_DEFECT"
    return "NEW_UNRELATED_HISTORICAL_DEFECT"


def tire_runtime_status(cfg: PgConfig, db: str, last_applied: str | None, full_success: bool) -> dict[str, str]:
    ...
    return {
        "slot13_reached": "PASS" if full_success or reached_migration(last_applied, SLOT13_REPAIR) else "NOT_REACHED",
        "r3b1g_repair_reached": status(R3B1G_REPAIR_MIGRATION),
        "r3b1g_repair_applied": status(R3B1G_REPAIR_MIGRATION),
        "migration_157_reached": status(TIRE_CONSUMER),
        "migration_157_applied": status(TIRE_CONSUMER),
    }
...
def run_full_replay(db_name: str = "synqdrive_r3b1g_full_replay") -> dict:
    cfg = PgConfig()
    ...
    while True:
        ...
        partial = {
            "PRE_R3B1G_SHA": PRE_SHA,
            "BASE_R3B1F111_SHA": BASE_R3B1F111_SHA,
            ...
            "tire_runtime": tire_runtime_status(cfg, db_name, last_applied, False),
            ...
            "final_status": "CI_R3B1G_TIRE_STATUS_REPAIR_FULL_REPLAY_FAILED"
            if failure_class == "R3B1G_REPAIR_DEFECT"
            else "CI_R3B1G_TIRE_STATUS_REPAIR_FULL_REPLAY_PARTIAL",
        }
    ...
    result = {
        ...
        "tire_runtime": tire_runtime_status(cfg, db_name, last_applied, full_success),
        ...
        "final_status": "CI_R3B1G_TIRE_STATUS_REPAIR_FULL_REPLAY_COMPLETED"
...
def main() -> int:
    db = sys.argv[1] if len(sys.argv) > 1 else "synqdrive_r3b1g_full_replay"
