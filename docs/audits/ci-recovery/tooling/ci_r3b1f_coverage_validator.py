#!/usr/bin/env python3
"""Expression dependency coverage validator for CI-R3B1F."""
...
def coverage_for_migration(mig: str, matrix_records: list[dict]) -> dict:
    ...
    for stmt_order, stmt in enumerate(split_sql_statements(sql), 1):
        if not re.search(r"\b(CREATE\s+(?:UNIQUE\s+)?INDEX|CHECK\s*\(|GENERATED\s+ALWAYS|USING\b|UPDATE\s+\")", stmt, re.I):
            continue
        parsed_indexes = []
        for parsed in __import__("expression_dependency_extractor", fromlist=["parse_create_index"]).parse_create_index(stmt):
            table = parsed["table"]
            predicate = parsed.get("predicate")
            cols_part = parsed.get("cols_part") or ""
