...
| **16** | 1 | Battery V2 Job-ID Sanitization | Prompt 4 | Backend / Workers | Kein `:` in BullMQ custom job id; Prod-Log-Fehler behoben; FHS-T-092 | TODO | — | Repro: `battery-v2-bullmq-job-id.repro.spec.ts`; Sanitizer: `bullmq-job-id.sanitizer.spec.ts` (11 pass) — Producer noch nicht verdrahtet | Revert Producer |
