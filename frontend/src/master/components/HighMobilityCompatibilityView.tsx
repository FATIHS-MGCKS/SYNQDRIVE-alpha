...
function card(isDark: boolean): string {
  return ` border ${isDark ? 'surface-premium border-border' : 'surface-premium border-border'}`;
}
  ...
  return (
    <div className="flex flex-col h-full min-h-0 px-4 sm:px-6 py-5 space-y-5">
      ...
      <div className="flex items-start gap-3 flex-wrap">
        <div className={`p-2 ${isDarkMode ? 'surface-premium' : 'bg-status-info-soft'}`}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-5">
        ...
        {resultLoading && (
          <div className={` border px-6 py-10 text-center text-sm text-muted-foreground ${isDarkMode ? 'bg-neutral-900 border-neutral-800' : 'surface-premium border-border'}`}>
