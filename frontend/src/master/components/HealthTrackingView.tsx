...
function BatterySection({ d }: { d: boolean }) {
  ...
  const code = (s: string) => (
    <code className={`px-1 py-0.5 rounded ${d ? 'surface-premium' : 'bg-muted'} ${mono}`}>
      {s}
    </code>
  );
      ...
      <div className={`${CARD(d)} p-5`}>
        ...
        <div className={`mt-3 grid grid-cols-6 gap-1 text-center`}>
          {[['12.73+', '100%'], ['12.62', '90%'], ['12.50', '80%'], ['12.37', '70%'], ['12.24', '60%'], ['12.10', '50%'],
            ...
            <div key={v} className={`rounded-lg p-1.5 text-[10px] ${d ? 'surface-premium' : 'bg-muted'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            {[
              ...
              <span key={f} className={`inline-block text-[10px] font-mono mr-1 mb-1 px-1.5 py-0.5 rounded ${d ? 'surface-premium text-muted-foreground' : 'bg-muted text-muted-foreground'}`}>{f}</span>
            ))}
          </div>
          <div>
...
function ErrorCodesSection({ d }: { d: boolean }) {
  const mono = `text-[11px] font-mono ${d ? 'text-violet-400' : 'text-violet-600'}`;
  const code = (s: string) => (
    <code className={`px-1 py-0.5 rounded ${d ? 'surface-premium' : 'bg-muted'} ${mono}`}>{s}</code>
  );
  const h2 = `text-sm font-semibold mb-2 ${d ? 'text-foreground' : 'text-foreground'}`;
  const body = `text-xs leading-relaxed ${d ? 'text-muted-foreground' : 'text-muted-foreground'}`;
...
function BrakeHealthSection({ d }: { d: boolean }) {
  ...
  const code = `text-xs font-mono px-1.5 py-0.5 rounded ${d ? 'surface-premium text-amber-300' : 'bg-muted text-amber-700'}`;
  const li = `text-sm ${d ? 'text-muted-foreground' : 'text-muted-foreground'}`;
  const th = `text-xs font-semibold uppercase tracking-wider px-3 py-2 text-left ${d ? 'text-muted-foreground surface-premium' : 'text-muted-foreground bg-muted'}`;
...
function TireHealthSection({ d }: { d: boolean }) {
  ...
  const h3 = `text-sm font-semibold mb-2 ${d ? 'text-muted-foreground' : 'text-foreground'}`;
  const p = `text-sm leading-relaxed mb-3 ${d ? 'text-muted-foreground' : 'text-muted-foreground'}`;
  const code = (t: string) => <code className={`px-1.5 py-0.5 rounded text-xs font-mono ${d ? 'surface-premium text-amber-400' : 'bg-muted text-amber-700'}`}>{t}</code>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${CARD(d)} p-6`}>
        ...
        <div className={`${d ? 'surface-premium' : 'bg-muted'} rounded-lg p-4 text-xs font-mono leading-relaxed space-y-1`}>
      ...
      <div className={`${CARD(d)} p-6`}>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            ...
            <div key={f.label} className={`${d ? 'surface-premium' : 'bg-muted'} rounded-lg p-3`}>
      ...
      <div className={`${CARD(d)} p-6`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
          {[
            ...
            <div key={label} className={`flex justify-between ${d ? 'surface-premium' : 'bg-muted'} rounded px-2 py-1`}>
      ...
      <div className={`${CARD(d)} p-6`}>
        ...
        <div className="grid grid-cols-5 gap-2 text-xs text-center">
          {[
            ...
            <div key={h.label} className={`${d ? 'surface-premium' : 'bg-muted'} rounded p-2`}>
      ...
      <div className={`${CARD(d)} p-6`}>
        ...
        <div className="grid md:grid-cols-2 gap-3 text-xs">
          {[
            ...
            <div key={c.cat} className={`${d ? 'surface-premium' : 'bg-muted'} rounded p-2`}>
    ...
    <div className="space-y-3">
      ...
      {section('Purpose & Position', (
        <>
          ...
          <div className="mt-3 flex flex-wrap gap-1">
            ...
            {badge('Backend Only', 'bg-muted text-muted-foreground')}
            {badge('Trip-Finalized Only', 'bg-muted text-muted-foreground')}
            {badge('No New Telemetry Polling', 'bg-muted text-muted-foreground')}
          </div>
        </>
      ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {['distanceKm', 'citySharePercent', 'highwaySharePercent', 'countrySharePercent',
                ...
                <span key={f} className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${d ? 'surface-premium text-muted-foreground' : 'bg-muted text-muted-foreground'}`}>{f}</span>
              ))}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="space-y-1">
              {[
                ...
                <div key={name} className={`${d ? 'surface-premium' : 'bg-muted'} rounded px-3 py-2`}>
      ...
      {section('Rolling Aggregate — VehicleDrivingImpactCurrent', (
        <div className="mt-2 space-y-2 text-xs">
          ...
          <div className={`${d ? 'surface-premium' : 'bg-muted'} rounded px-3 py-2 font-mono text-[10px] ${d ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
      ...
      {section('Config Reference (driving-impact.config.ts)', (
        <div className={`mt-2 ${d ? 'surface-premium' : 'bg-muted'} rounded px-3 py-2 font-mono text-[10px] leading-relaxed ${d ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl text-left text-sm transition-all ${
                isActive
                  ? d ? 'surface-premium text-white' : 'bg-muted text-foreground'
                  : d ? 'text-muted-foreground hover:surface-premium hover:text-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
              }`}
            >
