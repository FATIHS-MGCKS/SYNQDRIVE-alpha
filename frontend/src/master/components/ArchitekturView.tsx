...
const HEALTH_CALCS: HealthCalcEntry[] = [
  { name: 'Brake Health V2 (Canonical Refactor)', icon: Disc, inputs: ['Brake lifecycle events (inspection/pads/discs/fluid/full) with optional measured snapshot', 'TripDrivingImpact rows since anchor (canonical temporal wear exposure)', 'VehicleDrivingImpactCurrent only as uncovered-gap fallback', 'Vehicle master (fuelType, brakeForceFrontPercent)', 'Odometer progression'], algorithm: 'Achsweise (front/rear). Pads: base_wear=usable_mm/70k. Discs: base_wear=2.0mm/90k. Effective rate = base × (bias/default) × usage × stopDensity × hardBrake × fullBraking × reku × k (disc adds highSpeed + thermal). Wear is accumulated incrementally per trip since anchor. Rolling profile is never applied retroactively to full post-anchor distance; it is only used for uncovered odometer gaps. State semantics: MEASURED / ESTIMATED / WARNING_ONLY / NO_BASELINE. Confidence score includes coverage quality and fallback penalties. CRITICAL: No modeled estimate without valid baseline anchor + odometer.', output: 'Canonical read model: pads/discs health, limitingComponent, remainingKm, coverageRatio, modeledDistanceKm, modeledTripCount, modelingSource, baseline/provenance warnings, structured service history semantics. Runtime consumers use /brake-health/summary + /brake-health/detail.', color: 'text-red-400' },
  { name: 'Tire Health V2 (Canonical Lifecycle + Wear Model)', icon: Disc, inputs: ['Canonical tire lifecycle commands (install, rotate, replace single/axle/full, activate stored set)', 'Canonical recordMeasurement pipeline (manual/workshop/ai_confirmed/calibration)', 'AI tire spec + tire condition + setup metadata', 'Driving Impact Engine V1', 'Vehicle master data', 'Trip temperature + speed', 'DIMO tire pressure (numeric) + HM pressure freshness/status context', 'k-factor calibration + regression data points'], algorithm: 'Wear math remains model-aware V2 (base × axle × usage × behavior × temperature × pressure × load × seasonMismatch × regen × k × interaction). Runtime architecture now routes every meaningful write through TireLifecycleService, emits coherent TireEvent/TirePositionHistory, and recalculates deterministically. Pressure handling is honesty-first: stale/incomplete pressure uses neutral wear fallback while HM freshness/status augments confidence/warnings/action semantics. Active setup resolution is standardized to status=ACTIVE + removedAt=null.', output: 'Operational read model: overallPercent, overallRemainingKm, actionState (OBSERVE/CHECK_SOON/PLAN_SERVICE/REPLACE), confidence, measurementState (measured/estimated/mixed), pressureContext freshness/source, dataQualityWarnings, alerts, per-wheel estimates, explainability.', color: 'text-amber-400' },
  ...
  { name: 'Battery (HV)', icon: Battery, inputs: ['Provider-reported DIMO SOH evidence', 'HV telemetry snapshots (SoC/range/temperature/charging state/power/current energy/gross capacity)', 'Energy throughput between SoC readings', 'Nominal capacity from vehicle master', 'Odometer + historical snapshots'], algorithm: 'Canonical precedence: provider_reported SOH (fresh) → telemetry_derived capacity/energy estimation → bounded model_derived fallback. Publication still runs through EWMA + hysteresis (HvBatteryHealthCurrent), while evidence is persisted in BatteryEvidence with observedAt/source provenance. Current battery telemetry remains explicitly separated from health in the canonical contract.', output: 'Canonical hv section: healthPercent, method, evidenceType, freshness, publication state, telemetry context, charging sessions, trend and interpretation', color: 'text-emerald-400' },
  { name: 'Battery (LV) V2', icon: Battery, inputs: ['DIMO lowVoltageBatteryCurrentVoltage (+ source timestamp)', 'Rest voltage (60m/6h)', 'Crank drop + recovery', 'Rest stability', 'Document-confirmed LV workshop measurements'], algorithm: 'Canonical LV path keeps V2 publication pipeline (raw → stabilized → published) and observedAt-aware capture. BatteryEvidence persists LV measurements and model/published SOH provenance. Canonical contract exposes honest LV runtime states (ready/calibrating/stabilizing/no_recent_data/estimate_unavailable) and calibration blockers; voltage, SoC, and range are never represented as battery health percentages.', output: 'Canonical lv section: healthPercent/estimatedHealthPercent, condition, method, confidence, freshness, calibration progress, LV telemetry + evidence provenance', color: 'text-cyan-400' },
...
const TRIP_FLOWS: TripFlowEntry[] = [
  ...
  { name: 'Canonical Trip Analytics + Assignment Truth', icon: Car, source: 'VehicleTrip canonical counters + TripDrivingImpact + booking overlap resolution + explicit assignment/private model', process: 'TripAnalyticsCanonicalService hydrates one read model with semantically clean total/hard counters, separated Driving Style/Safety scores, and assignment eligibility truth (`ASSIGNED_*`, `PRIVATE_UNASSIGNED`, `UNKNOWN_ASSIGNMENT`). Driver/User scoring aggregates only assigned non-private trips.', storage: 'VehicleTrip assignment fields + TripDrivingImpact scores + canonical API contracts (trip detail/list/stats/driver-score)' },
  { name: 'Driving Impact Engine V1', icon: Layers, source: 'VehicleTrip canonical counters (hardware-neutral) + TripBehaviorEvent/DrivingEvent braking context + speeding exposure signals', process: 'Per-100km rates, 5 stress scores, separated Driving Style (behavior-focused) and Safety (speeding-focused) score outputs, 30-day distance-weighted rolling aggregate, and compatibility drift observation for legacy score mirrors.', storage: 'TripDrivingImpact (canonical per-trip score source) + VehicleDrivingImpactCurrent (rolling, consumed by Tire/Brake) + score-drift Prometheus counters' },
];
