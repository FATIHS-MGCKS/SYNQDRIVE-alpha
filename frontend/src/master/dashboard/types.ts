...
export interface ResilienceStatusDto {
  ...
  postgres: { lastSuccessAt: string | null; status: 'ok' | 'stale' | 'failed' | 'unknown' };
  clickhouse: { lastSuccessAt: string | null; status: 'ok' | 'stale' | 'failed' | 'unknown' };
  offsite: { lastSyncAt: string | null; status: 'ok' | 'stale' | 'failed' | 'unknown' };
  restoreValidation: { lastRunAt: string | null; status: 'passed' | 'failed' | 'overdue' | 'unknown' };
...
export interface AdminBillingOverviewSnapshot {
  mrr: number;
...
export interface MasterDashboardOperationalDto {
  generatedAt: string;
  ...
  billing: AdminBillingOverviewSnapshot | null;
