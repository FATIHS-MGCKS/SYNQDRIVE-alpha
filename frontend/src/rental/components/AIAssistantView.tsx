...
import { Send, Sparkles, Plus, MessageSquare, Car, Calendar, DollarSign, Wrench, Users, ListTodo, ChevronRight, Copy, Check, RotateCcw, ThumbsUp, ThumbsDown, Loader2, Bot, User, AlertCircle, Trash2 } from 'lucide-react';
...
export function AIAssistantView({ isDarkMode }: AIAssistantViewProps) {
  ...
  const handleSend = useCallback(async (text?: string) => {
    ...
    try {
      const response = await api.chat.sendMessage(orgId, msg);
      ...
      const displayMessage = isNetworkError
        ...
        : err?.message?.includes('API error')
          ...
          : `I'm sorry, something went wrong. ${err?.message || 'Please try again.'}`;
                  ...
                  {suggestions.map(s => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <ChevronRight className={`w-3.5 h-3.5 ml-auto shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className={`rounded-lg px-3 py-2 ${isDarkMode ? 'bg-neutral-800' : 'bg-gray-50/80'}`}>
                    <div className="flex items-center gap-2">
                      <Loader2 className={`w-3.5 h-3.5 animate-spin ${isDarkMode ? 'text-purple-400' : 'text-purple-500'}`} />
                      <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{t('aiChat.thinking')}</span>
                    </div>
                  </div>
                </div>
              )}
