          ...
          <div className="space-y-2 flex-1 overflow-y-auto pr-1">
            ...
            {filteredCustomers.map((c) => (
              <button
                ...
                className={`w-full min-w-0 max-w-full text-left p-4 rounded-lg border transition-all duration-200 flex items-start gap-3 group/card sm:items-center ${ selectedCustomer?.id === c.id ? 'sq-tone-brand ring-1 ring-[color:var(--brand-glow)]' : 'bg-muted/40 hover:surface-premium hover:border-border' }`}
              >
        ...
        return (
          <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onCloseAddCustomer}>
            ...
            <div onClick={(e) => e.stopPropagation()}
              className={`relative w-full max-w-[680px] max-h-[85vh] flex flex-col rounded-lg border ${ 'surface-premium/90' }`}>
              {/* Header */}
                        ...
                        <div className="flex gap-2">
                          {(['Individual', 'Corporate'] as const).map(t => (
                            <button key={t} onClick={() => onNewCustomerChange({ ...newCustomer, type: t })}
                              className={`flex-1 py-2.5 rounded-lg border text-xs font-semibold transition-all ${ newCustomer.type === t ? 'bg-brand text-brand-foreground border-brand' : 'surface-premium text-muted-foreground hover:border-border' }`}>
                ...
                <div className="flex items-center gap-2.5">
                  {addStep > 0 && (
                    <button onClick={() => onAddStepChange(addStep - 1)}
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border text-xs font-medium transition-all ${ 'surface-premium text-foreground hover:bg-muted' }`}>
