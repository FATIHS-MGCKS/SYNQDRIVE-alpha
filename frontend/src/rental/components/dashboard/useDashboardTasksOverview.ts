...
export function useDashboardTasksOverview({
  ...
  const countsComplete = !stationScoped || !listQuery.hasMore;

  useEffect(() => {
    if (!stationScoped || !queryEnabled || listQuery.error) return;
    if (!listQuery.hasMore || listQuery.loadingMore) return;
    void listQuery.loadMore();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- paginate until ALL_OPEN is complete for station counts
  }, [
    stationScoped,
    queryEnabled,
    listQuery.hasMore,
  ...
  const counts = useMemo(() => {
    if (stationScoped) {
      if (!countsComplete || listQuery.error) return null;
  ...
  }, [activeTasks, stationScoped, stationTasks]);

  const loading = stationScoped
    ? listQuery.loading || (!countsComplete && !listQuery.error)
    : summaryQuery.loading || listQuery.loading;
