...
export function derivePredictiveOperationsInsights(input: {
  ...
  for (const r of pendingReturns) {
    ...
    push({
      ...
      timeLabel: followMs
        ? de
          ? dt(input.locale, 'dashboard.time.pickupAt', { time: new Date(followMs).toLocaleTimeString(dashboardFormattingLocale(input.locale), { hour: '2-digit', minute: '2-digit' }) })
          : undefined
        : undefined,
    ...
    if (vehicle && (runtimeState
      ...
      push({
        ...
        sourceData: de
          ...
          : `cleaningStatus=${vehicle.cleaningStatus} · pickup BK ${p.bookingId}`,
        recommendedAction: dt(input.locale, 'dashboard.insight.scheduleCleaning'),
        ...
        if (fuel != null && !isEv && fuel < LOW_FUEL_PERCENT) {
          push({
            ...
            sourceData: de
              ...
              : `fuelPercent=${fuel} · pickup BK ${p.bookingId}`,
            recommendedAction: dt(input.locale, 'dashboard.insight.verifyFuel'),
        ...
        if (soc != null && isEv && soc < LOW_EV_SOC_PERCENT) {
          push({
            ...
            sourceData: de
              ...
              : `evSoc=${soc} · pickup BK ${p.bookingId}`,
            recommendedAction: dt(input.locale, 'dashboard.insight.verifyCharge'),
            confidence: vehicle.isFresh === true ? 'medium' : 'low',
            timeSortMs: startMs ?? now,
            timeLabel,
            cta: 'open-vehicle',
