...
function buildMaintenanceItems(
  ...
  if (runtimeStates.length > 0) {
    for (const state of runtimeStates) {
      ...
      items.push({
        ...
        status: 'blocked',
        timeMs: now,
        timeLabel: dt(input.locale, 'dashboard.operations.now'),
        vehicleLabel: state.license || state.displayName,
        vehicleId: state.vehicleId,
        station: state.stationLabel || v?.station || undefined,
        risks: [reason?.title ?? dt(input.locale, 'dashboard.operations.maintenanceBlocks')],
        tone: 'watch',
        cta: 'open-vehicle',
        completed: false,
        sortPriority: 700,
  ...
  for (const v of input.fleetById.values()) {
    ...
    items.push({
      ...
      risks: [dt(input.locale, 'dashboard.operations.maintenanceBlocks')],
      tone: 'watch',
      cta: 'open-vehicle',
      completed: false,
      sortPriority: 700,
