import { useEffect, useMemo, useState } from 'react';
import { StatusChip } from '../../../components/patterns';
import { useLanguage } from '../../../i18n/LanguageContext';
import { Icon } from '../ui/Icon';
import type { PickupContextResult } from '../../lib/damage-pickup-context';
import type { DamageLiabilityStatus, DamageResponse } from '../../lib/damage.types';
import { DAMAGE_LIABILITY_OPTIONS } from '../../lib/damage.types';
import {
  formatDamageDateLocale,
  formatDamageEuroCents,
  resolveDamagePickupContextLabel,
  resolveDamagePickupReasonLabel,
  resolveDamageSourceLabel,
  resolveLiabilityStatusLabel,
  resolveMatchConfidenceLabel,
  resolveDamageValidationMessage,
} from '../../lib/rental-vehicle-damages-i18n';

interface DamageRentalSectionsProps {
  damage: DamageResponse;
  pickupContext: PickupContextResult;
  busy?: boolean;
  onUpdateLiability: (
    damage: DamageResponse,
    input: { liabilityStatus: DamageLiabilityStatus; liabilityNote?: string },
  ) => Promise<void>;
  onPrepareDepositHold: (damage: DamageResponse, cents: number) => Promise<void>;
  onPrepareCustomerCharge: (damage: DamageResponse, cents: number) => Promise<void>;
}

export function DamageRentalSections({
  damage,
  pickupContext,
  busy,
  onUpdateLiability,
  onPrepareDepositHold,
  onPrepareCustomerCharge,
}: DamageRentalSectionsProps) {
  const { t, locale } = useLanguage();
  const [liabilityStatus, setLiabilityStatus] = useState<DamageLiabilityStatus>(
    damage.liabilityStatus ?? 'NOT_APPLICABLE',
  );
  const [liabilityNote, setLiabilityNote] = useState(damage.liabilityNote ?? '');
  const [depositEuro, setDepositEuro] = useState(
    damage.depositHoldCents != null ? String(damage.depositHoldCents / 100) : '',
  );
  const [chargeEuro, setChargeEuro] = useState(
    damage.chargedToCustomerCents != null ? String(damage.chargedToCustomerCents / 100) : '',
  );
  const [sectionError, setSectionError] = useState<string | null>(null);

  useEffect(() => {
    setLiabilityStatus(damage.liabilityStatus ?? 'NOT_APPLICABLE');
    setLiabilityNote(damage.liabilityNote ?? '');
    setDepositEuro(damage.depositHoldCents != null ? String(damage.depositHoldCents / 100) : '');
    setChargeEuro(
      damage.chargedToCustomerCents != null ? String(damage.chargedToCustomerCents / 100) : '',
    );
    setSectionError(null);
  }, [
    damage.id,
    damage.liabilityStatus,
    damage.liabilityNote,
    damage.depositHoldCents,
    damage.chargedToCustomerCents,
  ]);

  const hasRentalContext = useMemo(
    () =>
      Boolean(
        damage.bookingId ||
          damage.customerId ||
          damage.handoverProtocolId ||
          damage.source !== 'MANUAL',
      ),
    [damage.bookingId, damage.customerId, damage.handoverProtocolId, damage.source],
  );

  const pickupLabel =
    pickupContext.context !== 'NOT_APPLICABLE'
      ? resolveDamagePickupContextLabel(t, pickupContext.context)
      : null;

  return (
    <div className="space-y-5">
      {hasRentalContext && (
        <section>
          <h4 className="sq-section-label mb-2">{t('vehicleDamages.rental.section.context')}</h4>
          <div className="rounded-lg border border-border/70 bg-muted/15 px-3 py-2.5 space-y-2 text-[12px]">
            <ContextRow label={t('vehicleDamages.drawer.field.source')} value={resolveDamageSourceLabel(t, damage.source)} />
            {damage.bookingId && (
              <ContextRow label={t('vehicleDamages.rental.field.booking')} value={damage.bookingId} mono />
            )}
            {damage.customerId && (
              <ContextRow label={t('vehicleDamages.rental.field.customer')} value={damage.customerId} mono />
            )}
            {pickupLabel && (
              <div className="flex items-center justify-between gap-2 pt-1">
                <span className="text-muted-foreground">{t('vehicleDamages.rental.field.pickupContext')}</span>
                <StatusChip
                  tone={
                    pickupContext.context === 'NEW_SINCE_PICKUP'
                      ? 'warning'
                      : pickupContext.context === 'NEEDS_REVIEW'
                        ? 'critical'
                        : 'neutral'
                  }
                >
                  {pickupLabel}
                </StatusChip>
              </div>
            )}
            {pickupContext.suggestedPickupDamageId && (
              <p className="text-[11px] text-muted-foreground">
                {t('vehicleDamages.rental.suggestedPickupMatch', {
                  id: pickupContext.suggestedPickupDamageId.slice(0, 8),
                  confidence: resolveMatchConfidenceLabel(t, pickupContext.matchConfidence),
                })}
              </p>
            )}
            <p className="text-[11px] text-muted-foreground">
              {resolveDamagePickupReasonLabel(t, pickupContext.reason)}
            </p>
          </div>
        </section>
      )}

      {damage.handoverProtocolId && (
        <section>
          <h4 className="sq-section-label mb-2">{t('vehicleDamages.rental.section.handover')}</h4>
          <div className="rounded-lg border border-border/70 bg-muted/15 px-3 py-2.5 text-[12px] space-y-1">
            <ContextRow label={t('vehicleDamages.rental.field.protocol')} value={damage.handoverProtocolId} mono />
            <ContextRow
              label={t('vehicleDamages.rental.field.recordedVia')}
              value={resolveDamageSourceLabel(t, damage.source)}
            />
            {damage.reportedBy && (
              <ContextRow label={t('vehicleDamages.rental.field.reportedBy')} value={damage.reportedBy} />
            )}
          </div>
        </section>
      )}

      <section>
        <h4 className="sq-section-label mb-2">{t('vehicleDamages.rental.section.liability')}</h4>
        <div className="rounded-lg border border-border/70 bg-muted/15 px-3 py-2.5 space-y-3">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[12px] text-muted-foreground">{t('common.status')}</span>
            <StatusChip
              tone={
                damage.liabilityStatus === 'DISPUTED' || damage.liabilityStatus === 'NEEDS_REVIEW'
                  ? 'warning'
                  : damage.liabilityStatus === 'CUSTOMER_RESPONSIBLE'
                    ? 'critical'
                    : 'neutral'
              }
            >
              {resolveLiabilityStatusLabel(t, damage.liabilityStatus)}
            </StatusChip>
          </div>
          <label className="block">
            <span className="text-[11px] text-muted-foreground">{t('vehicleDamages.rental.field.operatorDecision')}</span>
            <select
              value={liabilityStatus}
              disabled={busy}
              onChange={(e) => setLiabilityStatus(e.target.value as DamageLiabilityStatus)}
              className="mt-1 w-full rounded-lg border border-border/70 bg-background px-3 py-2 text-sm"
            >
              {DAMAGE_LIABILITY_OPTIONS.map((opt) => (
                <option key={opt} value={opt}>
                  {resolveLiabilityStatusLabel(t, opt)}
                </option>
              ))}
            </select>
          </label>
          <label className="block">
            <span className="text-[11px] text-muted-foreground">{t('vehicleDamages.rental.field.liabilityNote')}</span>
            <textarea
              value={liabilityNote}
              disabled={busy}
              onChange={(e) => setLiabilityNote(e.target.value)}
              rows={2}
              className="mt-1 w-full rounded-lg border border-border/70 bg-background px-3 py-2 text-sm resize-none"
              placeholder={t('vehicleDamages.rental.liabilityNotePlaceholder')}
            />
          </label>
          <button
            type="button"
            disabled={busy}
            onClick={() => {
              setSectionError(null);
              void onUpdateLiability(damage, {
                liabilityStatus,
                liabilityNote: liabilityNote.trim() || undefined,
              }).catch(() =>
                setSectionError(resolveDamageValidationMessage('LIABILITY_SAVE_FAILED', t)),
              );
            }}
            className="sq-press px-3 py-2 rounded-lg text-[11px] font-semibold border border-border/70 disabled:opacity-50"
          >
            {t('vehicleDamages.rental.saveLiability')}
          </button>
          {sectionError && <p className="text-[11px] text-red-600 dark:text-red-400">{sectionError}</p>}
        </div>
      </section>

      <section>
        <h4 className="sq-section-label mb-2">{t('vehicleDamages.rental.section.costDeposit')}</h4>
        <div className="rounded-lg border border-border/70 bg-muted/15 px-3 py-2.5 space-y-3 text-[12px]">
          <ContextRow
            label={t('vehicleDamages.rental.field.estimatedRepair')}
            value={formatDamageEuroCents(locale, damage.estimatedCostCents) ?? '—'}
          />
          <ContextRow
            label={t('vehicleDamages.rental.field.actualRepairCost')}
            value={
              damage.repairCostCents != null
                ? t('vehicleDamages.rental.actualRepairRecorded', {
                    amount: formatDamageEuroCents(locale, damage.repairCostCents) ?? '—',
                  })
                : '—'
            }
          />
          <ContextRow
            label={t('vehicleDamages.rental.field.depositHold')}
            value={formatDamageEuroCents(locale, damage.depositHoldCents) ?? t('vehicleDamages.rental.depositNotSet')}
          />
          <ContextRow
            label={t('vehicleDamages.rental.field.customerCharge')}
            value={
              damage.chargedToCustomerCents != null
                ? t('vehicleDamages.rental.chargePrepared', {
                    amount: formatDamageEuroCents(locale, damage.chargedToCustomerCents) ?? '—',
                  })
                : t('vehicleDamages.rental.chargeNotSet')
            }
          />
          <p className="text-[11px] text-muted-foreground">{t('vehicleDamages.rental.billingHint')}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <label className="block">
              <span className="text-[11px] text-muted-foreground">{t('vehicleDamages.rental.field.prepareDeposit')}</span>
              <input
                type="text"
                inputMode="decimal"
                value={depositEuro}
                disabled={busy || !damage.bookingId}
                onChange={(e) => setDepositEuro(e.target.value)}
                className="mt-1 w-full rounded-lg border border-border/70 bg-background px-3 py-2 text-sm"
                placeholder={damage.bookingId ? '0.00' : t('vehicleDamages.rental.depositPlaceholder')}
              />
            </label>
            <label className="block">
              <span className="text-[11px] text-muted-foreground">{t('vehicleDamages.rental.field.prepareCharge')}</span>
              <input
                type="text"
                inputMode="decimal"
                value={chargeEuro}
                disabled={busy}
                onChange={(e) => setChargeEuro(e.target.value)}
                className="mt-1 w-full rounded-lg border border-border/70 bg-background px-3 py-2 text-sm"
                placeholder="0.00"
              />
            </label>
          </div>
          <div className="flex flex-wrap gap-2">
            {damage.bookingId && (
              <button
                type="button"
                disabled={busy || !depositEuro.trim()}
                onClick={() => {
                  const num = Number(depositEuro.replace(',', '.'));
                  if (!Number.isFinite(num) || num < 0) {
                    setSectionError(resolveDamageValidationMessage('DEPOSIT_AMOUNT_INVALID', t));
                    return;
                  }
                  setSectionError(null);
                  void onPrepareDepositHold(damage, Math.round(num * 100)).catch(() =>
                    setSectionError(resolveDamageValidationMessage('DEPOSIT_PREPARE_FAILED', t)),
                  );
                }}
                className="sq-press px-3 py-2 rounded-lg text-[11px] font-semibold border border-border/70 disabled:opacity-50"
              >
                {t('vehicleDamages.rental.prepareDeposit')}
              </button>
            )}
            <button
              type="button"
              disabled={busy || !chargeEuro.trim()}
              onClick={() => {
                const num = Number(chargeEuro.replace(',', '.'));
                if (!Number.isFinite(num) || num < 0) {
                  setSectionError(resolveDamageValidationMessage('CHARGE_AMOUNT_INVALID', t));
                  return;
                }
                setSectionError(null);
                void onPrepareCustomerCharge(damage, Math.round(num * 100)).catch(() =>
                  setSectionError(resolveDamageValidationMessage('CHARGE_PREPARE_FAILED', t)),
                );
              }}
              className="sq-press px-3 py-2 rounded-lg text-[11px] font-semibold border border-border/70 disabled:opacity-50"
            >
              {t('vehicleDamages.rental.prepareCharge')}
            </button>
          </div>
        </div>
      </section>

      <section>
        <h4 className="sq-section-label mb-2">{t('vehicleDamages.rental.section.evidencePackage')}</h4>
        <div className="rounded-lg border border-dashed border-border/70 px-3 py-3 text-[12px] text-muted-foreground space-y-1">
          <p className="flex items-center gap-1.5">
            <Icon name="camera" className="w-3.5 h-3.5" />
            {damage.images.length === 1
              ? t('vehicleDamages.rental.photoCount', { count: damage.images.length })
              : t('vehicleDamages.rental.photoCountPlural', { count: damage.images.length })}
          </p>
          {formatDamageDateLocale(locale, damage.reportedAt) && (
            <p>
              {t('vehicleDamages.rental.reportedAt', {
                date: formatDamageDateLocale(locale, damage.reportedAt)!,
              })}
            </p>
          )}
          {damage.reportedBy && (
            <p>{t('vehicleDamages.rental.reportedByLine', { name: damage.reportedBy })}</p>
          )}
          <p>
            {t('vehicleDamages.drawer.field.source')}: {resolveDamageSourceLabel(t, damage.source)}
          </p>
          {damage.bookingId && (
            <p>{t('vehicleDamages.rental.bookingLine', { id: damage.bookingId.slice(0, 8) })}</p>
          )}
          <p className="text-[11px] pt-1">{t('vehicleDamages.rental.pdfExportHint')}</p>
        </div>
      </section>
    </div>
  );
}

function ContextRow({
  label,
  value,
  mono,
}: {
  label: string;
  value: string;
  mono?: boolean;
}) {
  return (
    <div className="flex justify-between gap-3">
      <span className="text-muted-foreground">{label}</span>
      <span className={`text-foreground text-right ${mono ? 'font-mono text-[11px]' : ''}`}>
        {value}
      </span>
    </div>
  );
}
