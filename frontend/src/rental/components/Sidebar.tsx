    ...
    <>
      ...
      {expandedSections.includes('administration') && (
        <nav className="space-y-0.5 mb-1 animate-fade-up">
          ...
          <button onClick={() => { onSettingsTabChange?.('email-delivery'); handleViewChange('settings'); }} className={subNavBtnClass(currentView === 'settings' && settingsTab === 'email-delivery')}>
            <Mail className="w-[14px] h-[14px] shrink-0" /><span>{t('nav.emailDelivery')}</span>
          </button>
