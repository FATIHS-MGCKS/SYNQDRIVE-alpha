...
import { labelVoiceTab, localizedLaunchChecklistItem } from './voice-assistant-i18n';
import type { LaunchChecklistItem, VoiceTab } from './voice-assistant.ops';
...
export function VoiceLaunchChecklist({ items, onNavigate }: VoiceLaunchChecklistProps) {
  ...
  const localizedItems = items.map(item => localizedLaunchChecklistItem(locale, item));
  const required = localizedItems.filter(i => !i.optional);
  ...
  return (
    <section className="surface-premium rounded-2xl border border-border/40 p-4 shadow-[var(--shadow-1)] sm:p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-sm font-bold tracking-[-0.02em] text-foreground">{t('voice.launch.title')}</h2>
          <p className="mt-1 text-[11px] text-muted-foreground">{t('voice.launch.subtitle')}</p>
        </div>
        <StatusChip tone={complete === required.length ? 'success' : 'watch'} className="text-[10px]">
          {t('voice.launch.required', { complete, total: required.length })}
        </StatusChip>
      </div>
        ...
        {localizedItems.map(item => (
          ...
          >
            <div className="flex min-w-0 items-start gap-2.5">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {item.optional && (
                    <StatusChip tone="neutral" className="text-[9px]">{t('voice.launch.optional')}</StatusChip>
                  )}
                </div>
            ...
            {!item.ok && (
              ...
              >
                {t('voice.launch.fixIn', { tab: labelVoiceTab(locale, item.tab) })}
              </button>
            )}
          </div>
        ))}
