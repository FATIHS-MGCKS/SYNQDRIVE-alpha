...
export function VehicleAvailabilityInsights({
  ...
  const formattingLocale = vehicleFormattingLocale(locale);
  ...
  const content = (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0 space-y-2">
        <p className="sq-section-label">{t('vehicle.bookings.availability')}</p>
        <ul className="space-y-1.5" aria-label={t('vehicle.bookings.availabilityHints')}>
          ...
          {insights.length === 0 && nextFree && (
            <InsightRow
              insight={{
                ...
                tone: 'neutral',
                icon: 'calendar-clock',
                message: `Nächster freier Slot: ${formatSlotDurationLabel(nextFree.durationMs)} ab ${nextFree.start.toLocaleDateString(formattingLocale, { day: '2-digit', month: '2-digit' })}`,
              }}
            />
          )}
        </ul>
      ...
      {onCreateBooking && nextFree && nextFree.durationMs >= 24 * 60 * 60 * 1000 && (
        ...
        >
          ...
          {t('vehicle.bookings.bookingForSlot')}
        </button>
      )}
    </div>
  );
  ...
  if (embedded) {
    return (
      <footer className={`${vb.divider} px-4 py-3 sm:px-5 bg-muted/5`} aria-label={t('vehicle.bookings.availabilityInsights')}>
        {content}
      </footer>
    );
  }
  ...
  return (
    <section className={`${vb.section} ${vb.sectionBodyTight}`} aria-label={t('vehicle.bookings.availabilityInsights')}>
      {content}
    </section>
  );
}
