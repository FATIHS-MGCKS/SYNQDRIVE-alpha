...
export function VehicleBookingReadinessStrip({
    ...
    return (
      ...
      >
        <p className={`${vb.meta} font-medium`}>
          {t('vehicle.bookings.noUpcomingHandover')}
        </p>
      </div>
    );
  }
  ...
  const when = formatHandoverWhen(nextBooking, locale);
  const station =
    ...
    <section className={vb.section} aria-labelledby="vb-readiness-title">
      <header className={`${vb.sectionHeader} ${vb.sectionBodyTight}`}>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <p className="sq-section-label">{t('vehicle.bookings.nextHandover')}</p>
            ...
            {detailError && (
              <p className="text-[11px] text-[color:var(--status-attention)] mt-1.5" role="alert">
                {t('vehicle.bookings.detailPartiallyLoaded')}
              </p>
            )}
          ...
          <div className="flex flex-wrap gap-1.5 shrink-0" role="group" aria-label={t('vehicle.bookings.handoverActions')}>
            {onOpenFullBooking && (
              <button
                type="button"
            ...
            {showDocuments && (
              <button
                ...
                onClick={() => onOpenFullBooking!(nextBooking.id)}
              >
                {t('vehicle.bookings.checkDocuments')}
              </button>
            )}
            {showPickupPrep && (
              <button
      ...
      <div className={vb.sectionBodyTight}>
        {detailLoading && !detail ? (
          ...
          <p className={vb.meta}>{t('vehicle.bookings.noReadinessCheckpoints')}</p>
        ) : (
          <div
...
function formatHandoverWhen(booking: VehicleAgendaBooking, locale: string): string {
