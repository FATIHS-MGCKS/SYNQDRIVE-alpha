        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {data && !loading && (businessInsights.length > 0 || vehicleHealthAlerts.length > 0) && (
              <p className={`text-[10.5px] mt-0.5 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
        ...
        const cardBg =
          alert.severity === 'critical'
            ...
            : alert.severity === 'warning'
              ...
              : dm ? 'bg-blue-900/15 border-blue-800/30' : 'bg-blue-50/80 border-blue-200/60';
          ...
          >
            <div className="flex items-center justify-between gap-2 mb-1">
              <div className="flex items-center gap-2 min-w-0">
                <span className={`text-[10.5px] font-semibold truncate ${dm ? 'text-gray-100' : 'text-gray-900'}`}>
                  {alert.license || alert.model || alert.vehicleId.slice(0, 8)}
                </span>
                {alert.model && alert.license && (
                  <span className={`text-[10.5px] truncate ${dm ? 'text-gray-500' : 'text-gray-500'}`}>
                    {alert.model}
                  </span>
                )}
              </div>
            ...
            <p className={`text-[10.5px] leading-relaxed ${dm ? 'text-gray-200' : 'text-gray-800'}`}>
              {alert.primaryReason}
            </p>
            {alert.station && (
          ...
          <li
            ...
            className={`flex items-start gap-2.5 rounded-lg p-2.5 transition-all ${
              n.unread
                ...
                : dm ? 'bg-neutral-800/40' : 'bg-muted/50'
            }`}
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <span className={`text-[11.5px] font-semibold truncate ${dm ? 'text-gray-100' : 'text-gray-900'}`}>
                  {n.title}
                </span>
                {n.unread && <div className="w-1.5 h-1.5 rounded-full bg-violet-500 shrink-0" />}
              </div>
              <p className={`text-[10.5px] mt-0.5 leading-snug ${dm ? 'text-gray-400' : 'text-gray-600'}`}>
                {n.desc}
              </p>
            </div>
            <span className={`text-[10px] shrink-0 mt-0.5 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>{n.time}</span>
          </li>
        );
      })}
    </ul>
  ...
  return (
    ...
    >
      ...
      {hasExpandable && isExpanded && (
        <div
          className={`border-t px-3 py-2.5 space-y-2 ${dm ? 'border-neutral-800/40' : 'border-gray-200/60'}`}
    ...
    <ul className="space-y-1.5">
      {items.map((entity, idx) => {
        ...
        return (
          <li
            key={`${entity.id}:${idx}`}
            className={`flex items-center justify-between gap-2 rounded-md px-2 py-1.5 ${dm ? 'bg-black/20' : 'bg-white/70'}`}
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className={`text-[12px] font-semibold truncate ${dm ? 'text-gray-100' : 'text-gray-900'}`}>
                  {label}
                </span>
                {sub && (
                  <span className={`text-[10px] truncate ${dm ? 'text-gray-500' : 'text-gray-500'}`}>
                    {sub}
                  </span>
                )}
              </div>
              ...
              {secondaryReason && (
                <p className={`text-[10px] mt-0.5 leading-snug ${dm ? 'text-gray-500' : 'text-gray-500'}`}>
                  {secondaryReason}
                </p>
              )}
            </div>
...
function ExpandedGenericBlock({
  ...
  return (
    <div className="space-y-2">
      {(insight.reasons?.length ?? 0) > 0 && (
        <ul className={`text-[11px] leading-relaxed list-disc pl-4 space-y-0.5 ${dm ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      {targetView && onOpenView && (
        <button
          ...
          className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded-md transition-colors ${
            dm ? 'bg-blue-600/20 text-blue-300 hover:bg-blue-600/30' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'
          }`}
        >
