...
describe('stations permissions UI wiring', () => {
  ...
  it('uses useStationsV2Permissions in list and detail views', () => {
    ...
    const teamSource = readFileSync(resolve(stationsDir, 'StationTeamTab.tsx'), 'utf8');
    const activitySource = readFileSync(resolve(stationsDir, 'StationActivityTab.tsx'), 'utf8');
    expect(fleetSource).toContain('station-fleet-read-model.utils');
    expect(fleetSource).toContain('StationFetchStateBoundary');
    expect(teamSource).toContain('canManageTeam');
    expect(teamSource).toContain('scopeLabel');
    expect(activitySource).toContain('fromLabel');
    expect(activitySource).toContain('actor.displayName');
    expect(detailSource).toContain("activeTab === 'schedule'");
    expect(detailSource).toContain("activeTab === 'operations'");
    expect(detailSource).toContain('StationTeamTab');
    expect(detailSource).toContain('StationActivityTab');
