...
import { isTripAnalysisRunning } from './behavior-ui.utils';
...
export function TripBehaviorPanel({
  ...
  const analysisRunning = isTripAnalysisRunning(trip);

  if (loading || (analysisRunning && !behaviorIsReady && enrichStatus === 'PENDING')) {
    return <TripBehaviorEmptyState variant="pending" isDark={isDark} />;
  }

  if (analysisRunning && !behaviorIsReady) {
    return <TripBehaviorEmptyState variant="running" isDark={isDark} />;
  }

  if (trip.tripAnalysisStatus === 'SKIPPED' || enrichStatus === 'SKIPPED_NO_HF_DATA') {
    return <TripBehaviorEmptyState variant="skipped" isDark={isDark} />;
  }

  if (trip.tripAnalysisStatus === 'FAILED' || enrichStatus === 'FAILED_PERMANENT') {
