...
| `ELEVENLABS_API_KEY` | Runtime Secret | Voice Assistant + ElevenLabs MCP | As needed |
...
| **elevenlabs** | Env | `ELEVENLABS_API_KEY` — stdio via `uvx elevenlabs-mcp` (requires `uv`/`uvx` in the Cloud Agent image) |
